...
//! | `__crcbl_orbit_` | this module | boot, one rAF frame, teardown, logs |
//!
//! ## Exports
//!
//! | Symbol | Signature (wasm) | Meaning |
//! | --- | --- | --- |
//! | [`__crcbl_orbit_prepare`] | `() -> i32` | Install the log sink and the browser storage backends. **First call**, before any `__crcbl_web_fetch_*` or `__crcbl_web_opfs_*`. `1`, or `0` if it was already called. |
//! | [`__crcbl_orbit_log_level`] | `(i32) -> i32` | Set the log filter: `0` off … `5` trace. `1`/`0`. |
//! | [`__crcbl_orbit_boot`] | `() -> i32` | Open the shell on the canvas `__crcbl_web_canvas` announced, create the window, start the polled device request. `1`/`0`. |
//! | [`__crcbl_orbit_frame`] | `(f64) -> i32` | One `requestAnimationFrame`, given `performance.now()`. Returns the new status. |
//! | [`__crcbl_orbit_status`] | `() -> i32` | The status, without advancing anything. |
//! | [`__crcbl_orbit_shutdown`] | `() -> i32` | Tear the loop down. `1` if there was one. |
//! | [`__crcbl_orbit_error_ptr`] | `() -> i32` | Address of the last error message (UTF-8, not NUL-terminated), or `0`. |
//! | [`__crcbl_orbit_error_len`] | `() -> i32` | Its length in bytes. |
//! | [`__crcbl_orbit_log_take`] | `() -> i32` | Pop one log line into the scratch buffer and return its length; `0` when the queue is empty. |
//! | [`__crcbl_orbit_log_ptr`] | `() -> i32` | Address of that scratch buffer. Read it **after** `log_take`. |
//!
//! ## Status codes
//!
...
//! drawing, and it is a keystroke away from flying again. `FAILED` is the only
//! one that sets an error message.
//!
//! ## Call ordering
//!
//! ```text
//! __crcbl_orbit_prepare()                  // storage backends exist
...
//! __crcbl_orbit_boot()                     // shell + window; no size yet
...
//!   __crcbl_orbit_frame(performance.now()) // boot poll, or a frame
//!   __crcbl_orbit_log_take() … while non-zero
