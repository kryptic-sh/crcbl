//! orbit's one menu: the pause panel.
//!
//! ```text
//!   paused ──▶ Paused
//!   flying ──▶ none
...
//! this sample declares no action of its own — [`crate::app::Orbit`]'s
...
//!
//! A restart button would be the obvious fourth item and is deliberately not
//! here: a flight is started and re-started by the same control the rest of the
//! sample is flown with — the launch key, which [`crate::game::Controls`] calls
//! `launch` — and a menu button that sent it would be a second spelling of one
...
pub enum MenuKind {
    /// The flight is running: no menu at all.
    #[default]
    None,
    /// The loop has stopped advancing the flight.
    Paused,
}
...
/// orbit's menus, keyed by the state each belongs to.
