#!/usr/bin/env bash
# Run the flappy binary on a named backend, ask it for its last frame, and
# compare that frame against its checked-in golden.
#
#   CRCBL_GPU=vk apps/flappy/tests/run-flappy-golden.sh [extra nextest args…]
#
# # What this is for
#
...
#
# The suite is `apps/flappy/tests/golden.rs`, and what it drives is the
...
# a player would have seen, sky, course and title menu included.
#
...
crcbl_pin_vk_icd "flappy golden"
...
if [ -z "${CRCBL_GPU:-}" ]; then
    cat >&2 <<'NOBACKEND'
flappy golden: CRCBL_GPU is not set, so nothing would pin the backend and a
...
LOG="$(mktemp -t crcbl-flappy-golden.XXXXXX.log)"
cleanup() {
    local status=$?
    rm -f "$LOG" "${LOG}.plain"
    exit "$status"
...
set +e
cargo nextest run --locked -p flappy --features golden-e2e --test golden \
    --run-ignored all --no-tests fail --success-output immediate "$@" 2>&1 | tee "$LOG"
STATUS=${PIPESTATUS[0]}
set -e

if [ "$STATUS" -ne 0 ]; then
    echo "flappy golden: the suite failed on $CRCBL_GPU" >&2
    exit "$STATUS"
fi
...
ADAPTER="$(grep -F 'flappy golden: device on ' "${LOG}.plain" | head -1 || true)"
if [ -z "$ADAPTER" ]; then
    echo "flappy golden: the suite never named the adapter it drew on." >&2
...
echo "flappy golden: ${ADAPTER#*flappy golden: }"

echo "flappy golden: the course drew, made its claims and matched on $CRCBL_GPU"
