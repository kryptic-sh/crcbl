...
/// Nothing has been prepared; [`__crcbl_flappy_prepare`] has not run.
...
/// Playing. Every [`__crcbl_flappy_frame`] draws.
...
/// Something failed; [`__crcbl_flappy_error_ptr`] says what.
...
///
/// Relative to the *document*, so a demo served from `/crcbl/demos/flappy/`
/// fetches `/crcbl/demos/flappy/assets/<key>`. The trailing slash is required
...
/// The queue [`__crcbl_flappy_log_take`] drains.
#[derive(Default)]
struct LogQueue {
...
pub extern "C" fn __crcbl_flappy_prepare() -> u32 {
    ...
    with_app(0, |app| {
        ...
        log::info!("flappy: prepared; assets from {ASSET_BASE}");
    ...
    with_app(0, |app| {
        ...
        match PendingLoop::request(shell, &options, Clock::manual(Duration::ZERO)) {
            Ok(pending) => {
                ...
                log::info!("flappy: booting; waiting for the canvas to report a size");
                1
            }
            Err(error) => {
                app.fail(error);
    ...
    with_app(STATUS_FAILED, |app| {
        match core::mem::replace(&mut app.stage, Stage::Failed) {
            Stage::Booting(mut pending) => match pending.poll() {
                Ok(Some(engine)) => {
                    log::info!("flappy: running at {:?}", engine.extent());
    ...
    LOG.with(|slot| {
        ...
        match queue.lines.pop_front() {
            ...
            None => {
                ...
                let _ = write!(
                    queue.current,
                    "[WARN] flappy::web: {dropped} log lines dropped; the shim is not draining"
                );
            }
        }
