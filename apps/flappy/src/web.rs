...
impl WebLoop for Loop<dyn crcbl::shell::Shell> {
    type Error = crate::app::FlappyError;
...
impl WebPending for PendingLoop<dyn crcbl::shell::Shell> {
    ...
    fn request(
        ...
        clock: Clock,
    ) -> Result<Self, crate::app::FlappyError> {
        Self::request(shell, &Options::default(), clock)
    }

    fn poll(&mut self) -> Result<Option<Self::Loop>, crate::app::FlappyError> {
        Self::poll(self)
    }
}
...
///
/// Valid until the next export call. Read [`__crcbl_flappy_error_len`] first and
/// decode immediately.
#[unsafe(no_mangle)]
...
///
/// `0` means the queue is empty. Call [`__crcbl_flappy_log_ptr`] **after** this,
