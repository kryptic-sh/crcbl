...
/// the second binding of `game.rs`'s flap action, beside Space, and a panel on
/// screen shadows it. `crate::menu`'s header carries the argument, and the
/// primary binding (Space, the key the HUD names and every flap button prints)
/// is never taken.
///
/// They are consumed **only while a menu is showing**, so a run in the air is
/// flown with exactly the keys it always was.
