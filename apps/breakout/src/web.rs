...
/// Nothing has been prepared; [`__crcbl_breakout_prepare`] has not run.
...
/// Playing. Every [`__crcbl_breakout_frame`] draws.
...
/// `Stage` assignment in [`__crcbl_breakout_frame`] a multi-kilobyte move, once
...
/// The OPFS store the shim restored into, if [`__crcbl_breakout_prepare`] ran.
///
...
/// The asset source the shim pre-loads into, if [`__crcbl_breakout_prepare`] ran.
#[must_use]
...
/// The queue [`__crcbl_breakout_log_take`] drains.
#[derive(Default)]
struct LogQueue {
...
///
/// Valid until the next export call. Read [`__crcbl_breakout_error_len`] first
...
///
/// `0` means the queue is empty. Call [`__crcbl_breakout_log_ptr`] **after**
    ...
    LOG.with(|slot| {
        ...
        match queue.lines.pop_front() {
            ...
            None => {
                ...
                let _ = write!(
                    queue.current,
                    "[WARN] breakout::web: {dropped} log lines dropped; the shim is not draining"
                );
            }
        }
