...
#
# Every backend draws this board identically by construction — that is the point
...
crcbl_pin_vk_icd "breakout golden"
...
if [ -z "${CRCBL_GPU:-}" ]; then
    cat >&2 <<'NOBACKEND'
breakout golden: CRCBL_GPU is not set, so nothing would pin the backend and a
...
LOG="$(mktemp -t crcbl-breakout-golden.XXXXXX.log)"
cleanup() {
    local status=$?
    rm -f "$LOG" "${LOG}.plain"
    exit "$status"
...
# adapter, the two ratios it measured and the golden's own numbers — are only
# interesting on a green run, which is exactly the run nextest captures them on.
set +e
cargo nextest run --locked -p breakout --features golden-e2e --test golden \
    --run-ignored all --no-tests fail --success-output immediate "$@" 2>&1 | tee "$LOG"
STATUS=${PIPESTATUS[0]}
set -e

if [ "$STATUS" -ne 0 ]; then
    echo "breakout golden: the suite failed on $CRCBL_GPU" >&2
    exit "$STATUS"
fi
...
# this go red.
if [ "$CRCBL_GPU" = vk ] \
    && ! crcbl_validation_saw_nothing "${LOG}.plain" "the breakout golden suite"; then
    exit 1
fi

if ! crcbl_nextest_summary "${LOG}.plain" "breakout golden" \
...
ADAPTER="$(grep -F 'breakout golden: device on ' "${LOG}.plain" | head -1 || true)"
if [ -z "$ADAPTER" ]; then
    echo "breakout golden: the suite never named the adapter it drew on." >&2
...
echo "breakout golden: ${ADAPTER#*breakout golden: }"
