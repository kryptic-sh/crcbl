...
#
# The plaza is drawn identically on every backend by construction, so a run that
...
crcbl_pin_vk_icd "sundial golden"
...
if [ -z "${CRCBL_GPU:-}" ]; then
    cat >&2 <<'NOBACKEND'
sundial golden: CRCBL_GPU is not set, so nothing would pin the backend and a
...
echo "sundial golden: CRCBL_ADAPTER=${CRCBL_ADAPTER:-<unset>}"

LOG="$(mktemp -t crcbl-sundial-golden.XXXXXX.log)"
cleanup() {
    local status=$?
    rm -f "$LOG" "${LOG}.plain"
    exit "$status"
...
# selected paths, the adapter, and every penumbra, column count and darkening
# the shadow claims are made from — are only interesting on a green run, which
# is exactly the run nextest captures them on.
set +e
cargo nextest run --locked -p sundial --features golden-e2e --test golden \
    --run-ignored all --no-tests fail --success-output immediate "$@" 2>&1 | tee "$LOG"
STATUS=${PIPESTATUS[0]}
set -e

if [ "$STATUS" -ne 0 ]; then
    echo "sundial golden: the suite failed on $CRCBL_GPU" >&2
    exit "$STATUS"
fi
...
if [ "$CRCBL_GPU" = vk ] \
    && ! crcbl_validation_saw_nothing "${LOG}.plain" "the sundial golden suite"; then
    exit 1
fi

if ! crcbl_nextest_summary "${LOG}.plain" "sundial golden" \
...
ADAPTER="$(grep -F 'sundial golden: device on adapter ' "${LOG}.plain" | head -1 || true)"
if [ -z "$ADAPTER" ]; then
    echo "sundial golden: the suite never named the adapter it drew on." >&2
...
echo "sundial golden: ${ADAPTER#*sundial golden: }"
