//! Puppet's one menu: the pause panel.
//!
//! ```text
//!   paused ──▶ Paused
//!   running ─▶ none
...
//! this sample declares no action of its own — [`crate::app::Puppet`]'s
...
//!
//! Milestone 1 has nothing else to put on it. Everything a player can do here is
//! walking, and walking is keys rather than a row: there is no gait to pick, no
//! clip to scrub and no rig to inspect until milestone 2 brings one.
...
mod tests {
    ...
    /// [`crcbl::engine::MenuAction::from_id`] never asks puppet about an id.
    #[test]
    fn no_item_claims_an_id_the_game_would_have_to_answer_for() {
        ...
        for item in menus.current().expect("the paused menu").items() {
            ...
            assert_eq!(
                crcbl::engine::MenuAction::from_id(item.id, crate::app::Puppet::menu_action),
                Some(match item.id {
