//! Argument parsing for the tumble sample.
//!
//! ```text
//! tumble [--headless] [--frames N] [--size WxH]
//! ```
//!
...
//! those are the *engine's* vocabulary. This file is tumble's own: its usage
//! prose.
//!
//! # There is no flag of its own
//!
//! The scenes start the same way every run and take no input, which is what
//! makes the hash at `crate::scene::CHECK_TICK` a constant a native test and
//! the browser gate can both hold to. A flag that changed the start would be a
//! run whose hash nobody pinned.
...
/// What the command line asked tumble for.
...
impl Default for Options {
    fn default() -> Self {
        Self {
            ...
            common: Common::new(crate::scene::TICK_HZ).with_screenshot(),
            #[cfg(target_arch = "wasm32")]
            common: Common::new(crate::scene::TICK_HZ),
        }
    }
}
...
///
/// Every argument is offered to the shared set first, and one it does not take
/// is refused: tumble has no argument of its own.
...
mod tests {
    ...
    /// The engine tests the shared flags; what is tumble's to assert is that
    /// this sample's tick rate reaches them.
    #[test]
    fn the_defaults_are_a_windowed_run_at_the_scenes_tick_rate() {
        let options = parsed(&[]);
        assert!(!options.common.headless);
        assert_eq!(options.common.tick_hz, crate::scene::TICK_HZ);
    ...
    fn the_shared_half_of_the_usage_text_is_the_engines_verbatim() {
        ...
        assert!(USAGE.contains("tumble — the physics gallery"));
    }
}
