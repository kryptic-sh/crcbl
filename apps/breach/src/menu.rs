//! Breach's one menu: the pause panel.
//!
//! ```text
//!   paused ──▶ Paused
//!   running ─▶ none
...
//! this sample declares no action of its own — [`crate::app::Breach`]'s
...
pub enum MenuKind {
    /// The range is being shot: no menu at all.
    #[default]
    None,
...
/// Breach's menus, keyed by the state each belongs to.
...
mod tests {
    ...
    /// [`crcbl::engine::MenuAction::from_id`] never asks breach about an id.
    #[test]
    fn no_item_claims_an_id_the_game_would_have_to_answer_for() {
        ...
        for item in menus.current().expect("the paused menu").items() {
            ...
            assert_eq!(
                crcbl::engine::MenuAction::from_id(item.id, crate::app::Breach::menu_action),
                Some(match item.id {
