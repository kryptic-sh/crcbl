...
pub struct Gpu {
    ...
    /// presence in this string *is* "the page was drawn".
    #[cfg(test)]
    last_dump: String,
}
...
impl Gpu {
    ...
    /// measure with the *same* atlas the pass draws with or every measured
    /// string lands off by the difference.
    #[must_use]
    ...
    pub fn frame(&mut self) -> Result<FrameOutcome, GpuError> {
        ...
        let compiled = {
            ...
            // The only pass that does not load — see `CLEAR`.
            graph
                .add_render_pass("backdrop")
                .clear_color(target, CLEAR)
                .execute(|_| {});
        ...
        if !self.dumped {
            crcbl::log::debug!("render graph for bracket:\n{}", compiled.dump());
            self.dumped = true;
        }
