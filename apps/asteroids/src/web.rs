...
//!
//! # This is `apps/flappy/src/web.rs` with a different prefix, for the third time
//!
//! S1B **finding 2** said the browser entry point "should be written once,
//! before S2, which will otherwise write it a third time". This is that third
//! time, and the deadline was missed rather than met — deliberately, and the
//! reason is worth stating because it is not "we ran out of room".
//!
//! Sharing this file means putting it somewhere all three samples can reach:
...
//! as arguments. That is a change to `crates/`, to `apps/breakout` and to
//! `apps/flappy`: an engine-API slice wearing a sample slice's clothes, landing
//! in the same commit as this game's audio, its save file and its demo page. The
//! JS half was done as its own piece of work for exactly that reason, and the
//! Rust half deserves the same.
//!
//! **What the third copy now costs**, which is the part `docs/backlog.md` has
//! been updated to carry: the fix is no longer "write it once and adopt it in
//! two places". It is one new shared implementation plus **three** call sites to
//! migrate, three sets of `STATUS_*` constants to delete, three prefixes to
//! thread through a macro, and three browser gates to re-run. Every further
//! sample adds one more. The JS side settles the shape question in the
...
//! blocking wait would be sitting inside. [`__crcbl_asteroids_frame`] therefore
...
/// Nothing has been prepared; [`__crcbl_asteroids_prepare`] has not run.
...
/// Playing. Every [`__crcbl_asteroids_frame`] draws.
...
/// Something failed; [`__crcbl_asteroids_error_ptr`] says what.
...
///
/// Relative to the *document*, so a demo served from `/demos/asteroids/`
/// fetches `/demos/asteroids/assets/<key>`. The trailing slash is required by
...
/// `Stage` assignment in [`__crcbl_asteroids_frame`] a multi-kilobyte move, once
...
impl App {
    ...
    fn fail(&mut self, error: impl core::fmt::Display) -> u32 {
        ...
        crcbl::log::error!("asteroids: {}", self.error);
...
/// The OPFS store the shim restored into, if [`__crcbl_asteroids_prepare`] ran.
///
...
/// The asset source the shim pre-loads into, if [`__crcbl_asteroids_prepare`]
/// ran.
#[must_use]
...
/// The queue [`__crcbl_asteroids_log_take`] drains.
#[derive(Default)]
struct LogQueue {
...
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_prepare() -> u32 {
    install_logger();
    with_app(0, |app| {
        ...
        crcbl::log::info!("asteroids: prepared; assets from {ASSET_BASE}");
...
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_log_level(level: u32) -> u32 {
...
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_boot() -> u32 {
    with_app(0, |app| {
        ...
        match PendingLoop::request(shell, &options, Clock::manual(Duration::ZERO)) {
            Ok(pending) => {
                ...
                crcbl::log::info!("asteroids: booting; waiting for the canvas to report a size");
                1
            }
            Err(error) => {
                app.fail(error);
...
fn finish(app: &mut App, engine: Box<Loop<dyn crcbl::shell::Shell>>, reason: ExitReason) -> u32 {
    match engine.finish(reason) {
        Ok(summary) => {
            crcbl::log::info!(
                "asteroids: {} frames, {} ticks, score {} on wave {} ({:?}, {:?})",
                summary.frames,
                summary.ticks,
                summary.score,
                summary.wave + 1,
...
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_status() -> u32 {
...
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_shutdown() -> u32 {
    with_app(0, |app| {
...
///
/// Valid until the next export call. Read [`__crcbl_asteroids_error_len`] first
/// and decode immediately.
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_error_ptr() -> *const u8 {
...
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_error_len() -> u32 {
...
///
/// `0` means the queue is empty. Call [`__crcbl_asteroids_log_ptr`] **after**
/// this, not before: the two together are one read, and the buffer's contents
/// belong to the most recent `take`.
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_log_take() -> u32 {
    LOG.with(|slot| {
        let Ok(mut queue) = slot.try_borrow_mut() else {
            return 0;
        };
        ...
        match queue.lines.pop_front() {
            ...
            None => {
                ...
                let _ = write!(
                    queue.current,
                    "[WARN] asteroids::web: {dropped} log lines dropped; the shim is not draining"
                );
            }
        }
...
#[unsafe(no_mangle)]
pub extern "C" fn __crcbl_asteroids_log_ptr() -> *const u8 {
