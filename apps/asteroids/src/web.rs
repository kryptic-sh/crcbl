...
impl WebLoop for Loop<dyn crcbl::shell::Shell> {
    type Error = crate::app::AsteroidsError;
...
impl WebPending for PendingLoop<dyn crcbl::shell::Shell> {
    ...
    fn request(
        ...
        clock: Clock,
    ) -> Result<Self, crate::app::AsteroidsError> {
        Self::request(shell, &Options::default(), clock)
    }

    fn poll(&mut self) -> Result<Option<Self::Loop>, crate::app::AsteroidsError> {
        Self::poll(self)
    }
}
...
///
/// Valid until the next export call. Read [`__crcbl_asteroids_error_len`] first and
/// decode immediately.
#[unsafe(no_mangle)]
...
///
/// `0` means the queue is empty. Call [`__crcbl_asteroids_log_ptr`] **after** this,
