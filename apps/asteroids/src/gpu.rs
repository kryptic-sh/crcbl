...
//!
//! `space` (clear) → `sprites` (the game) → `ui` (the HUD, the menu and the debug
//! panel). The last two load rather than clear. The menu is drawn by the UI
//! pass after the game: its scrim dims what is already in the target, and its
//! labels go into the draw list after its art.
...
mod tests {
    ...
    fn the_features_this_sample_asks_for_are_the_engine_s_own() {
        ...
        assert_eq!(asked.label, "asteroids");
        assert_eq!(
