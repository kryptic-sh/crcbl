...
if [ -z "${CRCBL_GPU:-}" ]; then
    cat >&2 <<'NOBACKEND'
horde golden: CRCBL_GPU is not set, so nothing would pin the backend and a
...
LOG="$(mktemp -t crcbl-horde-golden.XXXXXX.log)"
cleanup() {
    local status=$?
    rm -f "$LOG" "${LOG}.plain"
    exit "$status"
...
set +e
cargo nextest run --locked -p horde --features golden-e2e --test golden \
    --run-ignored all --no-tests fail --success-output immediate "$@" 2>&1 | tee "$LOG"
STATUS=${PIPESTATUS[0]}
set -e

if [ "$STATUS" -ne 0 ]; then
    echo "horde golden: the suite failed on $CRCBL_GPU" >&2
    exit "$STATUS"
fi
...
# this go red.
if [ "$CRCBL_GPU" = vk ] \
    && ! crcbl_validation_saw_nothing "${LOG}.plain" "the horde golden suite"; then
    exit 1
fi

if ! crcbl_nextest_summary "${LOG}.plain" "horde golden" \
...
ADAPTER="$(grep -F 'horde golden: device on ' "${LOG}.plain" | head -1 || true)"
if [ -z "$ADAPTER" ]; then
    echo "horde golden: the suite never named the adapter it drew on." >&2
...
echo "horde golden: ${ADAPTER#*horde golden: }"
