...
// the docs.
//
// The sample owns its `__crcbl_<sample>_*` prefix, so the driver is told which
// one it is driving rather than having breakout's name baked in.
//
// Usage:
//   node web/tools/smoke.mjs <path-to.wasm> --sample <name>
  ...
  for (const descriptor of WebAssembly.Module.imports(module)) {
    ...
    switch (descriptor.kind) {
      ...
      case 'table':
        ...
        break;
      case 'memory':
        imports[descriptor.module][descriptor.name] = new WebAssembly.Memory({
          initial: 1,
        });
        break;
      default:
...
async function main() {
  ...
  // `apps/<sample>/src/web.rs` documents.
  const P = `__crcbl_${sample}_`;
  ...
  function drainLog() {
    const lines = [];
    for (;;) {
      const len = ex[`${P}log_take`]();
      if (len === 0) return lines;
      lines.push(
        decoder.decode(new Uint8Array(memory.buffer, ex[`${P}log_ptr`](), len))
      );
    }
  }
  ...
  check(
    ex.__crcbl_web_fetch_key_ptr() === 0,
    'the fetch scratch is absent before prepare'
  );
  check(
    ex.__crcbl_web_opfs_key_ptr() === 0,
    'the OPFS scratch is absent before prepare'
  );

  // ---- prepare -----------------------------------------------------------
  check(ex[`${P}prepare`]() === 1, 'prepare succeeds');
  check(ex[`${P}status`]() === STATUS_PREPARED, 'status is PREPARED');
  check(ex[`${P}prepare`]() === 0, 'a second prepare is refused');
  check(
    drainLog().some((line) => line.includes('prepared')),
    'the log queue delivers the line prepare wrote'
  );
  check(ex[`${P}log_take`]() === 0, 'a drained log queue answers 0');
  ...
  check(
    keyPtr !== 0 && keyCapacity >= 64,
    'the key scratch has an address and a capacity'
  );
  check(
    ex.__crcbl_web_key_scratch_ptr() === keyPtr,
    'the key scratch address is stable'
  );
  ...
  check(
    fetchPtr !== 0 && fetchCapacity > 0,
    'the fetch scratch exists after prepare'
  );
  ...
  new Uint8Array(
    memory.buffer,
    ex.__crcbl_web_fetch_key_ptr(),
    escape.length
  ).set(escape);
  check(
    ex.__crcbl_web_fetch_begin(escape.length) === 0,
    'a key that escapes the root is refused'
  );

  // ---- OPFS restore ------------------------------------------------------
  check(
    ex.__crcbl_web_opfs_environment(1) === 1,
  ...
  check(ex[`${P}boot`]() === 1, 'boot opens the shell and the window');
  check(ex[`${P}status`]() === STATUS_BOOTING, 'status is BOOTING');
  check(
  ...
  new Uint8Array(memory.buffer, keyPtr + code.length, composed.length).set(
    composed
  );
  ex.__crcbl_web_key(
    1,
  ...
  // `${P}frame` after a size would request a device, and a device
