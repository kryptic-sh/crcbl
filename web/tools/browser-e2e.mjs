...
try {
  ...
  // page's canvas, a swapchain configured on it (a `configure` with COPY_SRC in
  // the usage), an acquire (`getCurrentTexture`), a pass that clears the acquired
  // view to red, a copy of that frame into a host buffer, a submit, a present (a
  // no-op the browser composites on rAF), and a readback; the demo's loop replays
  // it; and the 16384 bytes that come back are asserted to be red, every pixel. A
  // stub that skipped the configure/acquire/render leaves a black/zero canvas and
  // fails here; only the real path — `context.configure`, `getCurrentTexture`,
  // render, `copyTextureToBuffer` off the acquired texture — leaves red.
  ...
  const presentStart = await evaluate(
    ...
    `(async () => {
       ...
       const { exports, gpu } = globalThis.crcbl;
       return { started: startPresentProbe({ exports, canvasId: gpu.canvasId }) };
     })()`
  );
