...
# Which browser GPU backend the site renders through. `wgpu` is the default and
# runs the full gate; `webgpu` builds with the umbrella's `webgpu` feature (via
# `CRCBL_WEB_BACKEND`, which `web/build.sh` reads) so the demo renders through
# `crcbl-webgpu`, and the crcbl-webgpu PROBE groups (G onward) are skipped —
# there the engine's own device owns the one command-stream channel, so the
# probes cannot install a second and `browser-e2e.mjs` does not run them. The
# named-check guards for those groups are skipped below to match.
...
# The crcbl-webgpu PROBE-group guards (groups G onward). In webgpu mode the
# engine's own device owns the one command-stream channel, so browser-e2e.mjs
# skips those groups and none of these lines is emitted — asserting them would
# fail a passing run. Groups A–F (the demo rendering through the backend, the
# isolation and touch guards above) run in both modes.
