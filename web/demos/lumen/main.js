...
// shared: this sample's `__crcbl_lumen_*` symbols, and the two strings the
// status bar shows.
//
...
// from, and `apps/lumen` integrates the free camera whether or not it is the one
...
import init from './crcbl_lumen.js';
...
bootDemo({
  ...
  savedLabel: 'Nothing',
  bind: (ex) => ({
    prepare: () => ex.__crcbl_lumen_prepare(),
    boot: () => ex.__crcbl_lumen_boot(),
    frame: (/** @type {number} */ now) => ex.__crcbl_lumen_frame(now),
    status: () => ex.__crcbl_lumen_status(),
    shutdown: () => ex.__crcbl_lumen_shutdown(),
    logLevel: (/** @type {number} */ level) =>
      ex.__crcbl_lumen_log_level(level),
    logTake: ex.__crcbl_lumen_log_take,
    logPtr: ex.__crcbl_lumen_log_ptr,
    errorPtr: () => ex.__crcbl_lumen_error_ptr(),
    errorLen: () => ex.__crcbl_lumen_error_len(),
  }),
});
