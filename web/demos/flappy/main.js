// Flappy in the browser: the page's half of the boot sequence and the rAF
// loop that drives it.
//
// The order below is not arbitrary — `apps/flappy/src/web.rs` specifies it,
...
/** Mirrors the `STATUS_*` constants in `apps/flappy/src/web.rs`. */
const STATUS = {
  IDLE: 0,
  PREPARED: 1,
  BOOTING: 2,
 ...
 * error: flappy's meshes, its font atlas and its shaders are all compiled
...
function lastError(exports, memory) {
  return readUtf8(
    memory,
    exports.__crcbl_flappy_error_ptr(),
    exports.__crcbl_flappy_error_len()
  );
}
...
async function main() {
  ...
  const log = () =>
    drainLog({
      memory,
      take: exports.__crcbl_flappy_log_take,
      ptr: exports.__crcbl_flappy_log_ptr,
    });

  if (exports.__crcbl_flappy_prepare() !== 1) {
    say('The engine refused to start.', lastError(exports, memory), true);
    log();
    return;
  }
  exports.__crcbl_flappy_log_level(LOG_INFO);
  ...
  if (exports.__crcbl_flappy_boot() !== 1) {
    say(
  ...
  window.addEventListener('pagehide', () => {
    ...
    exports.__crcbl_flappy_shutdown();
    flush();
  });
  ...
  function frame(now) {
    ...
    const status = exports.__crcbl_flappy_frame(now);
    log();
  ...
  Object.defineProperty(globalThis, 'crcbl', {
    value: {
      exports,
      memory,
      status: () => exports.__crcbl_flappy_status(),
      ...
      logLevel: (/** @type {number} */ level) =>
        exports.__crcbl_flappy_log_level(level),
    },
  });
}
