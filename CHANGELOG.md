...
- **Rotating rigid bodies and the tumble sample** (`36-contact-solver.md` rung
  ...
  specified IEEE operations, within `MAX_KERNEL_ULP` of the true value and
  bit-identical on every target. `crcbl-phys`'s `RigidBody` gains angular
  velocity, torque and an inertia tensor, built by `MassProperties`; the
  gyroscopic term is integrated by the implicit midpoint rule, which holds a
  free body's momentum and energy where implicit Euler bleeds them.
  `SurfaceMaterial` and `CombineRule` ride on each body through
  `PhysicsSystem::set_material`, with `apply_torque` alongside, and
  `crates/crcbl-phys/clippy.toml` denies platform transcendentals in the crate.
  `apps/tumble` and its web demo show a T-handle flipping in zero g and a box
  dropped flat gaining no spin, and its browser gate holds the wasm build's
  state hash to the native pin.

- **A field of card grass, generated by compute and drawn beside the opaque
  frame** — `docs/plan/57-grass.md`'s rung G1.
  `ForwardRenderer::set_grass(device, queue, Some(&field))` makes a
  `crcbl_render::grass::GrassField` resident — bounds, a ground heightfield, one
  cover map carrying density and blade row, and a table of blade types — and
  every view then records three passes: a clear, one generation dispatch per
  tile, and one indirect draw per slot, each slot reached by a dynamic uniform
  offset so `firstInstance` stays zero for WebGPU. A renderer with no field
  records none of them and draws the frame it drew before, bit for bit.
  **Placement is an integer hash**, so which cells carry a blade is the same
  bits on the CPU and on every backend, and
  `crcbl_render::grass::placement::cover_at` answers "is this position in grass,
  and how tall" from the static maps and the blade row's authored height — never
  from a blade, never from the wind. Cards are two crossed quads built from
  `SV_VertexID` with no vertex stream, cut out against a coverage-preserving mip
  chain cooked by Castaño's bisection, and lit through guarded copies of the
  mesh shader's cascade and punctual walks. `ForwardRenderer::set_wind_layers`
  and `set_wind` carry the wind field's two layers and its uniform block to the
  GPU, and the generation pass leans each blade at its root: a zero-intensity
  texel leaves the lean exactly zero. `crcbl screenshot --scene meadow` draws
  the fixture.
...
- **A grass card reads a whole mip level chosen from its own width**, not one
  ...
  a level at the angles a field of cards is made of, and an alpha cutout turns
  half a level into a binary per-pixel decision — 10.2% of `Scene::Meadow`'s
  pixels past `Tolerance::RASTERISER` between two conformant drivers, against
  0.05% with the level chosen per instance.
...
- **`PhysicsSystem` stores bodies in dense generational sets** instead of hash
  ...
  as one value, and covers the rotation fields, so state hashes differ from
  earlier builds.
...
- **The base-colour page carries a mip chain.** `crcbl_render::mip` builds one
  ...
  recorded with. The player's key is `[engine.video] anisotropic_filtering`:
  `crcbl::settings::anisotropic_filtering` reads it — `1` off, up to
  `MAX_ANISOTROPIC_FILTERING` (sixteen, the desktop preset's), the default when
  absent, the first key that may ask for more than the engine's default —
