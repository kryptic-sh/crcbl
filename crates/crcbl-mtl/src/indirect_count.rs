...
/// `indirectBufferOffset` as a multiple of four. It is also what lets every
/// offset this module handles become a **word index** in the kernel's uniform
/// block, so no buffer has to be bound at an offset at all.
pub(crate) const INDIRECT_OFFSET_ALIGNMENT: u64 = 4;

/// Bytes of one `MTLDrawPrimitivesIndirectArguments`: four 32-bit fields.
///
/// Fixed by the API, and identical to Vulkan's `VkDrawIndirectCommand`; see
/// `crcbl_mtl::draw`'s module docs for why the two agreeing is what lets one
/// compute pass feed both backends.
pub(crate) const DRAW_ARGS_BYTES: u64 = 16;
...
/// than written there, so the layout is stated once, on this side, where the
/// two structure sizes above already are.
