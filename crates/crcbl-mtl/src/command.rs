...
enum RenderCommand {
    ...
    /// `executeCommandsInBuffer:withRange:` over the indirect command buffer a
    /// queued [`crate::icb::Encode`] filled — the whole of a
    /// [`draw_indirect_count`](CommandEncoder::draw_indirect_count) on the
    /// render encoder's side.
    ///
    /// The range is the ICB's whole `maxCommandCount` rather than the count the
    /// GPU wrote, and that is the design: the kernel resets every command past
    /// the count, and a reset command is a no-op. So the number of draws that
    ...
    ExecuteIndirectCount {
        ...
        /// The ICB's `maxCommandCount`; see the variant docs.
        commands: NSUInteger,
...
fn replay(encoder: &ProtocolObject<dyn MTLRenderCommandEncoder>, command: &RenderCommand) {
    match command {
        ...
        RenderCommand::ExecuteIndirectCount {
            ...
            // not be bounds-checked. The kernel that wrote it ran on a compute
            // encoder this same command buffer ended before this render encoder
            // was opened, so the write is ordered ahead of the read; the ICB is
            // kept alive by the `Retained` this command holds; and the range is
            // the whole `maxCommandCount` it was created with, every command of
            // which that dispatch either encoded or reset.
            unsafe {
                encoder.executeCommandsInBuffer_withRange(icb, NSRange::new(0, *commands));
            }
        }
...
impl MetalCommandEncoder {
    ...
    fn indirect_count(&mut self, draw: &DrawIndirectCount, indexed: bool) {
        ...
        let commands = to_ns(u64::from(encode.commands()));
