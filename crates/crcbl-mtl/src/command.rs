//! [`MetalCommandEncoder`] — an encoder that records nothing and says so at
...
//!
//! **Nothing here is documented as working.** The recording methods accept
//! their arguments and drop them, exactly as they read, and the encoder cannot
//! produce a [`CommandBufferHandle`]: `finish` returns
//! [`HalError::Unsupported`] naming the slice that will implement it. A caller
//! therefore gets a "not yet" at the point it tries to *use* the recording
//! rather than a command buffer that submits and draws nothing — which is the
//! failure `crcbl_hal::Device::take_error` exists to catch on WebGPU, and which
//! is worth not reproducing on purpose here.
//!
//! What lands in the command slice is the real thing: an `MTLCommandBuffer`
//! from the device's queue, with render, compute and blit encoders opened
//! against it, and `finish` handing back a pooled handle. At that point this
//! file is replaced rather than extended.
...
pub(crate) struct MetalCommandEncoder {
    /// Zero-sized state today. The field exists so the type is a `struct` with
    /// a private constructor rather than a unit type anything could build, and
    /// so the command slice has somewhere to put the `MTLCommandBuffer`.
    _private: (),
}
...
impl CommandEncoder for MetalCommandEncoder {
    ...
    fn finish(self: Box<Self>) -> Result<CommandBufferHandle, HalError> {
        Err(MetalInstance::not_yet(
            "command recording (the Metal command slice)",
        ))
    }
}
