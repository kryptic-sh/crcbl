...
//!    of `max_draw_count` commands, and queues an [`Encode`].
...
//!    `executeCommandsInBuffer:withRange:` over the whole ICB. The commands
//!    past the count do nothing, so the number of draws that *happen* is the
//!    number the GPU wrote.
//!
...
use objc2_metal::{
    ...
    MTLLibrary, MTLPrimitiveType, MTLResource, MTLResourceID, MTLResourceUsage, MTLSize,
};
...
#define CRCBL_LIVE(index, params, counts) \
    ((index) < min((counts)[0], (params).max_draws))
...
[[kernel]] void crcblEncodePlainDraws(
    ...
    if (!CRCBL_LIVE(index, params, counts)) { command.reset(); return; }
...
impl Encode {
    ...
    /// # Errors
    ///
    /// Whatever [`allocate`] reports — a nil ICB or a nil argument buffer.
    ...
    /// How many commands the render encoder should execute — the ICB's whole
    /// range, since every command in it is written by this dispatch.
    pub(crate) const fn commands(&self) -> u32 {
        self.params.max_draws
    }
