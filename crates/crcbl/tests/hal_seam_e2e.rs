...
/// The geometry `mesh_shader.slang`'s mesh stage pulls, the layout it reads it
/// through, and the module both mesh exercises build their pipelines from.
///
/// Separate from [`Raster`] rather than folded into it because the two want
/// different things of one bind-group layout: the raster fixture's entry is
/// visible to [`ShaderStages::VERTEX`](crcbl::hal::ShaderStages::VERTEX) and a
/// mesh stage cannot read it, while an entry visible to
/// [`ShaderStages::MESH`](crcbl::hal::ShaderStages::MESH) is one
/// `ShaderStages::check_supported` refuses outright on a device that reports no
/// [`Features::MESH_SHADER`]. The colour target, the pass and the readback are
/// still `Raster`'s — this owns only the pieces that differ.
struct MeshFixture {
    ...
    pipeline_layout: crcbl::hal::PipelineLayoutHandle,
    module: crcbl::hal::ShaderModuleHandle,
    /// Whether [`module`](Self::module) is the mesh artifact or the raster one
    /// standing in for it, which decides what a refusal downstream is evidence
    /// of. See [`MeshFixture::open`].
    module_is_the_mesh_artifact: bool,
}

impl MeshFixture {
    /// Stages the three vertices in and builds the layout and the module.
    ///
    /// # The module is the mesh artifact only where there is a mesh stage
    ///
    /// **A device without [`Features::MESH_SHADER`] is never asked to compile
    /// `mesh_shader.slang`**, and the choice is made from the reported flag
    /// rather than from whatever error a refusal came back as.
    ///
    /// The reason is that the refusal would not be about this capability.
    /// **WGSL cannot express a mesh stage at all** — Slang refuses `meshMain`
    /// for that target by name, which is why the file's `crcbl-targets` line
    /// omits it — so `crcbl-wgpu` falls through to the SPIR-V and naga rejects
    /// it as `UnsupportedCapability(MeshShadingEXT)`, arriving as
    /// [`HalError::Backend`]. That is a sentence about a shader front end, and
    /// treating it as the capability's answer would mean the exercise scored a
    /// backend on which of its two artifact readers ran.
    ///
    /// The capability's own answer lives at `create_mesh_pipeline`, which every
    /// backend with no mesh stage refuses before reading a field of the
    /// descriptor. So such a device gets a module built from `triangle.slang`
    /// purely to have a handle the descriptor can name — and if a pipeline is
    /// nevertheless built out of it, both exercises fail loudly on
    /// [`module_is_the_mesh_artifact`](Self::module_is_the_mesh_artifact),
    /// because a mesh pipeline created on a device reporting no mesh stage is a
    /// backend performing what it declares absent.
    fn open(headless: &Headless) -> Self {
        let device = headless.device.as_ref();
        let features = device.caps().features;

        let vertices = raster_storage_buffer(
            headless,
            "mesh_shader vertices",
            &crcbl::shaders::mesh_shader::vertex_bytes(),
        );

        // `MESH` where there is a mesh stage to read the buffer, and `ALL`
        // where there is not. `ALL` is `VERTEX | FRAGMENT | COMPUTE` and names
        // no gated stage, so it is the visibility that survives
        // `check_supported` on a device without the feature — and on such a
        // device this layout exists only to reach the pipeline that refuses.
        // The same split `crcbl-vk`'s own mesh suite and `crcbl-dx12`'s made
        // separately, for the same reason.
        let visibility = if features.contains(Features::MESH_SHADER) {
            crcbl::hal::ShaderStages::MESH
        } else {
            crcbl::hal::ShaderStages::ALL
        };
        let layout_entries = [crcbl::hal::BindGroupLayoutEntry {
            binding: 0,
            visibility,
            // `StructuredBuffer<Vertex>` in the shader, so read-only here.
            kind: crcbl::hal::BindingKind::StorageBuffer {
                read_only: true,
                dynamic: false,
            },
        ...
        let bind_group_layout = device
            ...
            .expect("a one-buffer layout works on every tier");
        ...
        let module_is_the_mesh_artifact = features.contains(Features::MESH_SHADER);
        let module = if module_is_the_mesh_artifact {
            device
                .create_shader_module(&crcbl::hal::ShaderModuleDesc {
                    label: Some("mesh_shader.slang"),
                    spirv: crcbl::shaders::MESH_SHADER.spirv(),
                    wgsl: crcbl::shaders::MESH_SHADER.wgsl(),
                    msl: crcbl::shaders::MESH_SHADER.msl(),
                    dxil: &crcbl::shaders::MESH_SHADER.dxil_containers(),
                })
                .expect(
                    "this device reports MESH_SHADER, so the committed mesh artifact must \
                     compile here — a refusal is a missing or stale artifact rather than a \
                     capability answer, and mesh_shader.slang ships SPIR-V, MSL and DXIL",
                )
        } else {
            eprintln!(
                "crcbl hal seam e2e: this device reports no MESH_SHADER, so create_mesh_pipeline \
                 is asked with a stand-in module built from triangle.slang — the refusal it gives \
                 is the capability's answer, and a shader front end's opinion of mesh_shader.slang \
                 is not"
            );
            device
                .create_shader_module(&crcbl::hal::ShaderModuleDesc {
                    label: Some("triangle.slang, standing in for mesh_shader.slang"),
                    spirv: crcbl::shaders::TRIANGLE.spirv(),
                    wgsl: crcbl::shaders::TRIANGLE.wgsl(),
                    msl: crcbl::shaders::TRIANGLE.msl(),
                    dxil: &crcbl::shaders::TRIANGLE.dxil_containers(),
                })
                .expect("the raster artifact is accepted by every backend this suite runs")
        };
        ...
        Self {
            ...
            pipeline_layout,
            module,
            module_is_the_mesh_artifact,
        }
    }

    /// A mesh pipeline over this fixture's layout and module.
    ///
    /// Entry points are **named literals**, unlike every other pipeline in this
    /// file. `crcbl_shaders::Shader::entry_point` answers `None` when a stage
    /// has two entry points, and `mesh_shader.slang` declares two mesh ones on
    /// purpose — a mesh shader taking `in payload` requires a task stage
    /// upstream, so one entry point cannot serve both `task: None` and
    /// `task: Some(…)`. `crcbl-render`'s forward pass names its two the same way
    /// and for the same reason.
    ///
    /// # Errors
    ///
    /// Whatever `create_mesh_pipeline` returned, which on a backend with no mesh
    /// stage and on a device without the feature is the capability's refusal and
    /// is the point.
    fn pipeline(
        &self,
        device: &dyn Device,
        ...
        let color_targets = [crcbl::hal::ColorTargetState::opaque(RASTER_FORMAT)];
        let entry = |entry_point| crcbl::hal::ShaderEntry {
            module: self.module,
            entry_point,
        };
        device.create_mesh_pipeline(&crcbl::hal::MeshPipelineDesc {
            label: Some(label),
            layout: self.pipeline_layout,
            task: task.map(entry),
            mesh: entry(mesh),
            fragment: Some(entry("fragmentMain")),
            // `topology` is ignored for a mesh pipeline — the shader's
            // `[outputtopology]` decides it — and the rest of the defaults are
            // what every other pipeline in this file draws with.
    ...
    /// Draws one frame through `pipeline` and reads the target back.
    fn render(
        &self,
        headless: &Headless,
        raster: &Raster,
        ...
        raster.render(headless, label, None, |encoder| {
            encoder.bind_graphics_pipeline(pipeline);
            encoder.bind_group(0, self.bind_group, &[], self.pipeline_layout);
            // One workgroup of the *first* stage the pipeline has: the task
            // stage where there is one, the mesh stage otherwise. `taskMain`
            // dispatches one mesh workgroup of its own, and `meshMain` is three
            // threads emitting the one triangle either way.
...
/// Whether `read`'s centre texel is the corner the mesh triangle's apex carries.
///
/// **The claim that survives every convention this suite refuses to pin.**
/// `mesh_shader.slang`'s three corners are a saturated red apex and a green and
/// a blue base corner, and the target's centre sits nearest the apex: the
/// barycentric weight of the apex there is the larger of the three, so red is
/// the largest channel of the interpolated texel. That stays true under a Y flip
/// — `crcbl-vk` draws through a negative-height viewport and every other backend
/// does not — because the weight is a ratio of distances to the opposite edge
/// and both distances change sign together; and under an X mirror, because the
/// green and blue corners are symmetric about `x = 0` and only swap with each
/// other.
///
/// It is also a claim [`RASTER_BACKGROUND`] fails: that colour is
/// blue-dominant. So "the mesh stage's triangle reached the centre" is
/// separable from "the clear did", without this file learning the sign
/// convention its neighbours exist to check.
fn mesh_centre_is_the_red_apex(read: &RasterRead) -> bool {
    let centre = read.centre;
    centre[0] > centre[1] && centre[0] > centre[2]
}

/// Drives [`Capability::MeshShading`](crcbl::hal::Capability::MeshShading): a
/// mesh stage with no vertex stage in front of it emits a triangle that reaches
/// the colour target.
///
/// # What is asserted, and why a backend that does nothing cannot pass it
///
/// There is no vertex stage, no index buffer and no draw call with a vertex
/// range: the pipeline is a [`MeshPipelineDesc`](crcbl::hal::MeshPipelineDesc)
/// and the only thing recorded is `draw_mesh_tasks(1, 1, 1)`. Nothing else in
/// this suite can put a primitive on the target through that pair, so a texel
/// that stopped being [`RASTER_BACKGROUND`] was put there by a mesh stage.
///
/// A backend that accepted the pipeline and dropped the draw leaves the whole
/// target holding the clear, which is [`Exercise::SilentlyIgnored`] rather than
/// a pass — the arm this capability most needs, because a mesh draw that
/// vanishes produces a *plausible* frame on a renderer that has a lesser
/// geometry path to fall back to.
fn exercise_mesh_shading(headless: &Headless) -> Exercise {
    let device = headless.device.as_ref();
    let raster = Raster::open(headless);
    let fixture = MeshFixture::open(headless);

    let outcome = match fixture.pipeline(device, "mesh shading", None, "meshMain") {
        Err(error) => Exercise::Refused(error),
        Ok(pipeline) => {
            assert!(
                fixture.module_is_the_mesh_artifact,
                "this backend cannot compile mesh_shader.slang and still built a mesh pipeline \
                 out of triangle.slang's module — the stand-in exists only so a backend with no \
                 mesh stage can refuse the descriptor, and one that got this far has been asked a \
                 question about the wrong shader"
            );
            let read = fixture.render(headless, &raster, "mesh shading", pipeline);
            device.destroy_graphics_pipeline(pipeline);
            match read {
                Err(error) => Exercise::Refused(error),
                Ok(read) if read.drawn == 0 => {
                    eprintln!(
                        "crcbl hal seam e2e: the mesh pipeline was created, the draw was recorded \
                         and the target still holds nothing but the clear — {}",
                        read.diagnosis()
                    );
                    Exercise::SilentlyIgnored
                }
                Ok(read) if mesh_centre_is_the_red_apex(&read) => {
                    eprintln!(
                        "crcbl hal seam e2e: the mesh stage's triangle covered {} of {} texels — \
                         {}",
                        read.drawn,
                        RASTER_WIDTH * RASTER_HEIGHT,
                        read.diagnosis()
                ...
                Ok(read) => panic!(
                    "the mesh pass covered {} texels and the centre is not the red apex \
                     mesh_shader.slang puts there, so something drew and it was not this \
                     triangle: {}",
                    read.drawn,
                    read.diagnosis()
                ),
...
/// Drives
/// [`Capability::TaskShaderStage`](crcbl::hal::Capability::TaskShaderStage): an
/// amplification stage in front of the mesh stage delivers a payload the mesh
/// stage acts on.
///
/// # What is asserted, and why a backend that does nothing cannot pass it
///
/// **The stage is made observable rather than assumed.** `taskMain`'s payload is
/// [`AMPLIFICATION_TINT`](crcbl::shaders::mesh_shader::AMPLIFICATION_TINT),
/// which is `(0, 1, 1, 1)`, and `amplifiedMeshMain` multiplies every corner by
/// it — so a frame drawn through the amplification stage has the apex's red
/// killed and its green and blue untouched. Two frames are drawn on the same
/// device from the same vertices, differing only in whether the pipeline carries
/// a task stage, and the comparison is between *them* rather than against a
/// written-down colour: the interpolated texel depends on the sample point and
/// on the clip-space Y convention, and the difference the tint makes does not.
///
/// That rules out all three ways this could pass without the stage running. A
/// task stage dropped from the pipeline leaves the two frames identical, which
/// is [`Exercise::SilentlyIgnored`]. A payload that never arrived reads as zero
/// and blackens *every* channel, which the green-and-blue half of the
/// comparison catches. And a frame that never drew at all fails the plain arm's
/// own claim before the tinted one is looked at.
fn exercise_task_shader_stage(headless: &Headless) -> Exercise {
    let device = headless.device.as_ref();
    let raster = Raster::open(headless);
    let fixture = MeshFixture::open(headless);

    // The amplified pipeline first, because it is the one this capability is
    // about: a device with a mesh stage and no task stage refuses here, and
    // drawing the plain frame before finding that out would be work whose
    // result nothing reads.
    let outcome = match fixture.pipeline(
        device,
        "task shader stage",
        Some("taskMain"),
        "amplifiedMeshMain",
        ...
        Ok(amplified) => {
            assert!(
                fixture.module_is_the_mesh_artifact,
                "this backend cannot compile mesh_shader.slang and still built an amplified mesh \
                 pipeline out of triangle.slang's module — see exercise_mesh_shading"
            );
            let plain = fixture
                .pipeline(device, "task shader stage, plain", None, "meshMain")
                .expect("a device that took a task stage takes the mesh stage without one");
            let plain_read = fixture.render(headless, &raster, "mesh, no task stage", plain);
            let amplified_read =
                fixture.render(headless, &raster, "mesh with task stage", amplified);
            device.destroy_graphics_pipeline(plain);
            device.destroy_graphics_pipeline(amplified);

            match (plain_read, amplified_read) {
                (Err(error), _) | (_, Err(error)) => Exercise::Refused(error),
                (Ok(plain), Ok(amplified)) => {
                    // The plain arm's own claim, first: without it the tint
                    // comparison would be two blank frames agreeing about
                    // nothing.
                    assert!(
                        plain.drawn > 0 && mesh_centre_is_the_red_apex(&plain),
                        "the frame drawn without a task stage is not this triangle, so nothing \
                         can be read off the tinted one beside it: {}",
                        plain.diagnosis()
                    );
                    let tint = crcbl::shaders::mesh_shader::AMPLIFICATION_TINT;
                    let killed = expected_level(tint[0], RASTER_FORMAT);
                    let red_was_killed = amplified.centre[0].abs_diff(killed) <= LEVEL_TOLERANCE;
                    let rest_survived = (1..4).all(|channel| {
                        amplified.centre[channel].abs_diff(plain.centre[channel]) <= LEVEL_TOLERANCE
                    });
                    eprintln!(
                        "crcbl hal seam e2e: without the task stage the centre is {:?}, with it \
                         {:?}, and the tint {tint:?} predicts red at {killed} with the rest \
                         unchanged",
                        plain.centre, amplified.centre,
                    );
                    if red_was_killed && rest_survived {
                        Exercise::Worked
                    } else if amplified.centre == plain.centre {
                        eprintln!(
                            "crcbl hal seam e2e: the two frames are identical, so the task stage \
                             was accepted and contributed nothing — {}",
                            amplified.diagnosis()
                        );
                        Exercise::SilentlyIgnored
                    } else {
                        panic!(
                            "the amplified frame is neither the plain one nor the plain one with \
                             its red killed, so the payload that reached the mesh stage was not \
                             {tint:?}: {}",
                            amplified.diagnosis()
                        )
                    }
...
fn exercise(headless: &Headless, capability: crcbl::hal::Capability) -> Exercise {
    ...
    match capability {
        ...
        // Two exercises rather than one arm, because the second stage is a
        // separate capability and its observable is a *difference* between two
        // frames rather than a picture.
        C::MeshShading => exercise_mesh_shading(headless),
        C::TaskShaderStage => exercise_task_shader_stage(headless),
