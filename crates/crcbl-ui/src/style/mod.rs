...
//! initial values — with `color` and `font-size` inherited from its parent —
//! and then its builder's inline [`Declaration`]s on top. Rules from the
//! engine's [`DEFAULT_CSS`] sit below every app sheet's, whatever their
//! selectors; within one of those two origins a rule of a higher
//! [`Specificity`] tier beats a lower one, and within a tier the later rule
//! wins. Inline declarations beat every rule.
//!
