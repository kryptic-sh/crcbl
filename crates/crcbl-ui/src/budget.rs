...
///
/// Re-exported rather than defined here: `crcbl bench` reports the same
/// percentiles over its own samples, and the rank arithmetic and the count it is
/// honest at are one piece of knowledge. [`crcbl_core::stats`] is where it
/// lives, and where its derivation is tested across every count below it.
pub use crcbl_core::stats::MIN_PERCENTILE_SAMPLES;
