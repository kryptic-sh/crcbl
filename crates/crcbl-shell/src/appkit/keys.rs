...
/// which is exactly the set [`keysym_from_character`] answers for. The numbering
/// is X11's, which is what [`Keysym`] holds and what `crcbl_core::input`
/// explains the choice of.
///
/// **This table is the same fact as [`win32::keys::named_keysym`](crate::win32::keys::named_keysym)**,
/// because it is a mapping from the engine's own [`KeyCode`] to X11's numbering
/// with no platform in it at all. The two are checked against each other by a
/// test below that compiles wherever both modules do; `docs/backlog.md` carries
/// the extraction that would make the check unnecessary.
#[must_use]
...
mod tests {
    ...
    /// The two `KeyCode` → keysym tables are one fact.
    ///
    /// Compiled wherever both backends are, which under `cfg(test)` is every
    /// host — see `lib.rs`, which gates both on `any(target_os = …, test)` for
    /// exactly this kind of check. A table that is a pure function of the
    /// engine's own vocabulary has no platform in it, so the two cannot
    /// legitimately differ, and this is what stops them drifting until the
    /// extraction `docs/backlog.md` records is made.
    #[test]
    fn the_keysym_table_agrees_with_the_win32_backends_for_every_key() {
        for key in KeyCode::ALL {
            assert_eq!(
                named_keysym(*key),
                crate::win32::keys::named_keysym(*key),
                "{key} names a different symbol on the two backends"
            );
        }
    }
    ...
    fn named_keysyms_cover_the_keys_no_layout_can_move_and_no_others() {
        ...
        for key in [KeyCode::KeyA, KeyCode::Digit1, KeyCode::Space] {
