...
//! [`Keysym`], which is by definition layout-dependent. [`named_keysym`] answers
...
/// which is exactly the set `MapVirtualKeyW(.., MAPVK_VK_TO_CHAR)` answers for.
/// The numbering is X11's, which is what [`Keysym`] holds and what
...
/// [`Modifiers::ALT_GR`], out of [`modifiers`].
#[must_use]
...
mod tests {
    ...
    fn named_keysyms_cover_the_keys_no_layout_can_move_and_no_others() {
        ...
        // with `MapVirtualKeyW` clean rather than overlapping.
        for key in KeyCode::ALL {
