...
pub fn decode_delta(payload: &[u8]) -> Result<Delta, DeltaDecodeError> {
    ...
    let is_keyframe = payload[cursor] != 0;
    cursor += 1;
...
mod tests {
    ...
    fn snapshot_entities(snapshots: &[SystemSnapshot]) -> HashMap<u32, HashMap<u64, Vec<u8>>> {
        ...
        for snap in snapshots {
            out.insert(snap.system_id, decode_entity_blobs(&snap.data));
        }
        out
    }
    ...
    fn same_state_produces_empty_delta() {
        ...
        let baseline = Baseline::from_snapshot(tick_a, &snaps);
    ...
    fn add_one_entity() {
        ...
        let baseline = Baseline::from_snapshot(tick_a, &snaps_a);
    ...
    fn delta_apply_roundtrip() {
        ...
        let mut baseline = Baseline::from_snapshot(TickId::from_raw(1), &snaps_t1);
    ...
    fn delta_apply_removes_entities() {
        ...
        let mut baseline = Baseline::from_snapshot(tick_a, &snaps_a);
        let delta = DeltaCodec::encode(tick_b, &snaps_b, Some(&baseline));
    ...
    fn encode_decode_delta_roundtrip() {
        ...
        let baseline = Baseline::from_snapshot(TickId::from_raw(4), &snaps);
    ...
    fn encode_decode_delta_preserves_semantics() {
        ...
        let baseline = Baseline::from_snapshot(baseline_tick, &snaps);
