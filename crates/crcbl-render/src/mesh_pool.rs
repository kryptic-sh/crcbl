...
impl MeshPool {
    ...
    /// How many vertices `handle`'s mesh occupies in the vertex pool, or `None`
    /// on [`MeshPool::mesh`]'s terms.
    ///
    /// The fourth integer [`MeshRange`] deliberately does not carry, because a
    /// draw pulls through indices and needs none of it. A [`crate::skinning`]
    /// region does need it — a skinned run is exactly as long as the bind pose
    /// it deforms — and this is the only place the number exists.
    #[must_use]
