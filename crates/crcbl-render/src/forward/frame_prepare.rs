...
impl ForwardRenderer {
    ...
    pub(super) fn begin_frame_body(
        ...
        let mut views = std::mem::take(&mut self.shadow_prepare_views);
        let mut culls = std::mem::take(&mut self.shadow_prepare_culls);
        views.clear();
        culls.clear();
        let result = (|| {
            ...
            for assignment in self.shadow_lights.slots().iter().flatten() {
                match self.extra_lights.get(assignment.light) {
                    Some(Light::Spot(spot)) => {
                        light_view_proj[assignment.base] =
                            shadow::spot_matrix(spot).to_cols_array();
                    }
            ...
            views.reserve(SHADOW_VIEWS);
            culls.reserve(SHADOW_CULLS);
            ...
            // light's matrices.
            let mut face_culls: [Option<crate::cull::FacePlanes>; SHADOW_CULLS] =
                [None; SHADOW_CULLS];
        ...
        views.clear();
        culls.clear();
        self.shadow_prepare_views = views;
        self.shadow_prepare_culls = culls;
        result
    }
}
