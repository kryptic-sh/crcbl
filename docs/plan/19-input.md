...
| **On-screen controls** | **Built** — `crcbl_ui::touch`'s `TouchStick` and `TouchButton` hit-test raw contacts and report through `ActionMap::virtual_stick` / `virtual_button`, so a `Binding::Virtual` is a device like any other. Consumed by `apps/horde` | shipped                          |
...
| Slice                                                                                                                                                                                                                                                                                                                                      | Phase                                             |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| Action layer + kb/mouse — **built**, as `crcbl_input::ActionMap` over `ActionDecl`/`Binding`. **Patterns, contexts and the RON binding asset are not**, per the section above: an action carries no pattern list, there is no context stack, and nothing parses a binding file.                                                            | P2 (replaces the raw-input pipeline plan there)   |
