...
| Slice                                                                                                                                         | Phase                                                                                                                                                                                                                                                                                                                                               |
| --------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HDR target + exposure/tonemap pass                                                                                                            | P7 — built at P1; the ACES curve **built 2026-08-27** and auto-exposure **built 2026-08-29**, both per view, and the clamp stays the default — [48-post-processing.md](48-post-processing.md) says why                                                                                                                                              |
| Antialiasing rung 1: **FXAA 3.11**                                                                                                            | P7 — built, in `DEFAULT_STACK`, and the whole of the stack's AA row today                                                                                                                                                                                                                                                                           |
| Sun CSM (culling-integrated; the 3×3 PCF became the disc below)                                                                               | P7 — built. The **cascade debug overlay** is not                                                                                                                                                                                                                                                                                                    |
...
| ~~Shadow ladder rung 3: **rotated-disc PCF**~~                                                                                                | P7 — **built 2026-08-28**: a 32-tap Vogel disc over two tile texels, turned by an integer-indexed rotation table. A Vogel spiral rather than a Poisson set, for provenance. [45-shadows.md](45-shadows.md)'s ninth decision                                                                                                                         |
| ~~Shadow ladder rung 4: **PCSS**~~                                                                                                            | P7 — **built 2026-08-28**: a sixteen-tap blocker search sizes the filter per fragment, for the sun alone. The physical sun's penumbra is sub-texel here and was measured to be a no-op, so the shipped angle is artistic and says so. [45-shadows.md](45-shadows.md)'s tenth decision                                                               |
| ~~Shadow filter: **the disc's early-out**~~                                                                                                   | P7 — **built 2026-08-28**: five of the thirty-two taps decide whether the other twenty-seven are worth taking, so a fragment off a shadow edge costs 5 and a sun-lit one 21 against 48. No golden moved on either adapter. [45-shadows.md](45-shadows.md)'s eleventh decision                                                                       |
...
| ~~AO: **GTAO**~~                                                                                                                              | P10 — **built 2026-08-28**: a two-slice horizon integral replaced the hemisphere of depth comparisons, and it took a `crcbl_shaders::ssao::acos_approx` with it because no target specifies its own. Five goldens moved, every one of them a wash lifting off a flat surface. [46-ambient-occlusion.md](46-ambient-occlusion.md)'s delivery section |
...
| **Multi-scatter energy compensation** on the GGX lobe                                                                                         | **Built 2026-08-27**, ahead of the quality pass because it needed no new pass: the cooked `DFG` table, one image fetch and a multiply. [44-lighting.md](44-lighting.md)'s rung 1, and the table is the one specular IBL still needs                                                                                                                 |
| TAA (jitter, motion vectors); temporal SSR; ~~shadow atlases~~ (pulled forward 2026-08-30, [45-shadows.md](45-shadows.md))                    | post-MVP. Auto-exposure left this row on 2026-08-29 (post stack), and the `GpuInstance` slot it named is **reserved since 2026-08-27** — what TAA and temporal SSR still owe is the motion-vector pass                                                                                                                                              |
...
- **CSM artifact whack-a-mole** (peter-panning, acne, cascade seams): this risk
  ...
  overlay that was supposed to help is still unbuilt. **Most of it has now
  arrived**: the `cube` browser-path golden that had failed on linux and windows
  since the 2026-08-26 re-tiling is green again on both, and the seventh
  decision is what fixed it. What is left is the fringe that decision traded
  lantern's wall-foot strip for, at one silhouette, which `docs/backlog.md`
  still carries.
