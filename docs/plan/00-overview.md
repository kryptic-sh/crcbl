...
| Windowing    | From scratch (`crcbl-shell`, topic 15) — _bindings, not frameworks_              | Wayland/X11 protocol layer + codegen ours over libwayland-client/libxcb connections (the Vulkan WSI ABI requires real driver-visible objects — topic 15); own JS-shim canvas P5; Win32/AppKit at **P5C** (moved out of P14 on 2026-08-04). Platform-agnostic Shell trait; SurfaceTarget = only sanctioned leak. Two modes only: windowed (aspect-lockable) + borderless w/ render scale — the two modes exist in `crates/crcbl-shell`, but **the shell carries no render-scale request**: the renderer owns the knob and a settings key reads it, while `render_scale` appears nowhere in `crcbl-shell`. The `crcbl-shell` fractional scale is the compositor's HiDPI factor, a different quantity. See ROADMAP's known gaps. No exclusive fullscreen. |
...
| Wasm target  | WebGPU through our own command stream                                            | Browser has no Vulkan. **`crcbl-webgpu` is the backend, and it is not a `wgpu` wrapper**: wasm serialises HAL calls into a buffer it owns and JS decodes and replays them against WebGPU (its rules are in `docs/notes/browser.md`). This row named `wgpu` and `crcbl-wgpu` until 2026-08-23; that crate was deleted on 2026-08-21 and with it the native triage backend the row promised, so there is **no** portability fallback on native — `crcbl-vk`, `crcbl-mtl` and `crcbl-dx12` are the native set. Capability decides the path (topic 39). Web builds are single player; no browser networking.                                                                                                                                               |
...
| 43    | ◐ 43-render-standards — built part done, deleted 2026-09-24; the rest in `docs/backlog.md`  | What a current engine ships and where this one stands — the gap survey     |
...
| 49    | ◐ 49-antialiasing — built part done, deleted 2026-09-24; the rest in `docs/backlog.md`      | Antialiasing: FXAA, CMAA2, TAA's blocker, and MSAA repriced                |
| 50    | ◐ 50-irradiance-probes — built part done, deleted 2026-09-24; the rest in `docs/backlog.md` | Irradiance probes: the static L1 grid, the diffuse GI twin                 |
...
| 56    | [56-wind.md](56-wind.md)                                                                    | Wind: the two-layer field, gusts, motors and every consumer                |
| 57    | [57-grass.md](57-grass.md)                                                                  | Grass and vegetation wind: cards, blades, shells, swaying trees            |
| 58    | [58-hair.md](58-hair.md)                                                                    | Hair and fur: shells, cards on simulated chains, strands                   |
| 59    | [59-tessellation.md](59-tessellation.md)                                                    | Tessellation: displacement baked into clusters, patterns diced at run time |
...
tumble, relief). Nine of them no longer have a document of their own, all
deleted on 2026-09-24: breakout, asteroids, horde and flappy with every
milestone built, and viewer, quarry, sundial, alcove and options with their
built parts done. Their rules and recorded measurements are in
