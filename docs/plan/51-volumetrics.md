...
| **3 — a 3D target, a coarser grid and a depth-aware lookup** | The filtered lookup, and a grid an eighth of the frame across with sample count as a quality tier — the industry's answer is temporal reprojection, and it is refused here (2026-08-30): a history buffer makes the frame a function of how many frames preceded it, which every golden in the tree is built not to be | A volume in the transient pool, the engine's first 3D image on four backends, and a depth-aware upsample in the composite. **That was written expecting a shared pass and there is not one:** `shaders/ssao_upsample.slang` shipped 2026-09-02 AO-specific — an `Rgba8Unorm` target carrying one occlusion channel and a bent direction, its own `RESOLUTION_DIVISOR`, and `1.0` where nothing was drawn because unoccluded is that value's identity. A froxel lookup shares neither the channel layout nor that fallback, so this row owns generalising the pass or writing a second one, and either is a rung rather than a binding |
