...
- Shortest-roundtrip float formatting (Rust f64 `Display` semantics):
  deterministic and exact. **No custom float writer is needed and none was
  written**: ron 0.12.2's `Serializer::serialize_f32`/`serialize_f64` are
  `write!("{}", v)` plus a trailing `.0` when the value is integral, and Rust's
  float `Display` is shortest-round-trip out of `core`'s `flt2dec` — not out of
  a libm that varies by platform.
- **The trap that follows from it**: shortest-round-trip round-trips a _parsed_
  value, so a hand-typed `7.8000000000000005` is written back as `7.8` and fails
  a byte-compare that is about the writer rather than about the typist. A
  committed chunk file is therefore **generated by the writer once and
  maintained that way**, which is the discipline `crcbl_inventory::catalog`'s
  `CANONICAL` fixture already encodes.
...
4. Scene-dir format: deterministic RON writer, chunk load/save, roundtrip
   property tests — **landed 2026-09-07**, see the note at the foot of this
   file. Dirty-chunk tracking is the one item on the line that did not: it
   exists to make an editor's save cheap, there is no editor, and
   `crcbl_scene::scn::Scene::save` rewrites every chunk the manifest names.
...
6. `crcbl import` CLI wiring — the **report** half is built; see the task-3 note
   below and [11-cli-headless.md](11-cli-headless.md). Writing is no longer
   blocked — task 4 landed the directory to write into — and is not built:
   nothing in `crcbl-cli` calls `crcbl_scene::scn::Scene::save`, and
   `--out <dir>` is still refused by name. `crcbl bake` (may land later,
   pre-Pages-demo of a scene-heavy sample).
...
## Landed: task 4 — the `.scn/` directory (2026-09-07)

`crcbl_scene::scn`, in the crate whose package description has said "scene
format and glTF import" since the workspace skeleton. The two halves of that
description are now separable: `crcbl-scene`'s `gltf` feature is on by default
and gates every module spelled in terms of a glTF document (`gltf_import`,
`gltf_check`, `gltf_render`, `gltf_fixture` and `lod_resolve`, whose
`resolve_lod` takes a `&GltfScene`), the root `Cargo.toml`'s workspace entry
turns it off, and `crcbl`'s new non-default `scn` feature reaches the loader
with the parser left out.
`cargo tree -p crcbl --features scn --target wasm32-unknown-unknown -e normal`
names no `gltf` at all, which is what the `scene` feature's manifest comment has
always argued a shipped game should get.

- **Three file types, one per question.** `scene.ron` is
  `Scene(format, name, systems)` — the manifest is read in file order and a name
  with no registered codec is an error, not a skip; `env.ron` is the camera the
  scene opens on and its ambient light; `sys/<name>.ron` is
  `Chunk(system, entities)`, whose `system` field must agree with the file it
  was read as. Every one of the four serde types sets `deny_unknown_fields`, so
  a typo is a line and a column rather than a value silently defaulted.
- **`format: 0`, refused rather than migrated.** The rule the cooked-mesh
  subsection above fixes for every format the engine owns, applied to the first
  one that exists.
- **`SceneEntityId` is the file's id and is not the runtime handle.**
  `crcbl_core::Pool` has no insert-at-index, so an `Entity`'s bits are a
  function of spawn history; persisting them would mean regenerating ids on
  save, which the deterministic-writer section bans by name. `scn::IdMap` is the
  correspondence for as long as the scene is loaded, and it is a `BTreeMap`, so
  "entities sorted by stable ID" is the map's own ordering rather than a sort at
  save time.
- **`crcbl-ecs` gained nothing.** The `Serialize`/`DeserializeOwned` bound sits
  on `scn::chunk_of::<T>(name)` and not on `SystemTrait`, so the entity crate's
  dependency list is still `crcbl-core` and nothing else. Systems are reached by
  **name** — `World::schedule_mut` and `SystemTrait::as_any_mut` — because
  `World::system_mut::<T>` finds the first system of a type and a manifest names
  files. That trait exposes no shared `as_any`, which is the whole reason
  `SystemChunk::write` takes `&mut World` to change nothing.
- **The writer produces text, not files.** `Scene::save` returns the
  `scene.ron`, `env.ron` and `sys/<name>.ron` it would write, keyed relative to
  the scene directory; the caller puts them on disk. That is the "no synchronous
  IO anywhere in engine crates" exit criterion kept, and it is also what makes
  the three property tests below need no directory. Reading is the mirror:
  `Scene::load` takes a `&dyn AssetSource`, so `DirSource` is the native path
  and a `MemorySource` seeded from `include_str!` is the browser's.
- **The three property tests are `crates/crcbl-scene/tests/scn_roundtrip.rs`**,
...
6); the sidecar `.meta.ron` GUIDs of the Corrections section; the editor, its
command journal and the sector sharding of the "Scaling" subsection (P12).

> **Two earlier notes in this file are corrected by it, 2026-09-07.** The task-2
> note's "Nothing depends on the crate yet" was already overtaken; the task-3
> note's list of what is still absent said "no RON scene format" twice, and that
> half of it is now false. Everything else on those lists still holds: no GPU
> pool upload, no textures, no mip generation, no hot reload, and skins and
> animations are still deliberately unread.
