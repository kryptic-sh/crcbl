...
| **QEM auto-generation + meshlet cluster hierarchy** and `crcbl lod` — built. **Sidecar overrides were withdrawn** by this document's own text above, and skinned-awareness is unbuilt: `crcbl_scene::lod_resolve` says the hierarchy is over the bind pose and nothing there skins. This row still listed both until 2026-08-23 | **P7** (the `MeshShader` path selects per cluster and needs levels to select) |
