...
| **S1** — breakout | **done** | `d747a84` scaffold, `71d931c` paddle + input, `5989f1b` ball + physics, `495a7fb` bricks + scoring + game states, `ee3bea5` audio + HUD + e2e, `fa4e20b` spatial panning, `3bcf327` client interpolation, `a6c2e6b` high score, `ecfd85a` determinism tests                                                                                        |
...
- **`crcbl-cli`** (`new`/`run`/`build`) and **`apps/sandbox`**, which draws a
  ...
  Wayland and X11.
- **`apps/breakout`** (S1, 9 slices merged): paddle movement via `ActionMap`,
  ...
  `crcbl_store::write_atomic`, console HUD with score/lives/ball position, 6
  unit/e2e tests including scripted-input determinism. Remaining for exit:
  render-graph UI compositing pass for on-screen HUD.
...
**1198 unit/integration tests** (17 new from S1), plus 26 Vulkan e2e (run on
...
| **S1**     | **Sample: breakout** — first playable; all deps (P0–P4A) exist. **9 slices merged**: scaffold, paddle + input, ball + physics (swept-sphere CCD), bricks + scoring + game states, procedural audio (bounce/brick-break sine waves), spatial panning via `compute_cue`, client-interpolated ball state, high score persistence via `crcbl_store::write_atomic`, determinism tests. Remaining for exit criteria: render-graph UI compositing pass for on-screen HUD.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | —                 | Winnable/losable native game — 9 slices, no on-screen HUD yet                      |
