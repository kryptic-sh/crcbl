...
| **P7B**    | **The rasterised lighting twin, complete** (topic 18): spot and point shadows, screen-space AO, screen-space reflections, irradiance probes. Every one of the four is built and gated by a golden in `crates/crcbl/tests/render_e2e.rs`; the row stays unticked, and as of 2026-08-23 the reason is no longer lantern's to fix: `apps/lantern`'s room now has a spot light (`room::spot`) whose **cone draws and whose shadow does not**. `SHADOW_LIGHT_TILES` and `SHADOW_POINT_FACES` are the same number, so the atlas's light region is exactly one point light's cube and a room holding a point light and a spot gets one of their two maps — the lamp takes it, pinned on every frame of its orbit. Closing this row means growing the light region, which `docs/backlog.md` costs out. Lands **before** the ray-traced path — it is what macOS, iOS and every browser run                                                                                                                                                                                                                                                                                                   | 18                | lantern (13) renders the scene complete on `LightingPath::Rasterised`                                                                                          |
