...
`crates/crcbl-vk/tests/golden/` and `apps/lantern/tests/golden/`;
`CRCBL_BLESS=1` re-blesses; mismatches drop actual, expected and diff into
`target/golden-diff/` for CI to upload. Blessing deliberately returns an error
rather than passing, so a missing reference cannot silently switch a gate off.
...
- **lantern's browser gap does not close.** It is excluded from the browser gate
...
11. **The browser half is done.** `crcbl-wgpu` is a
    ...
    off every `.wasm` (breakout 3,182,323 → 1,609,109 bytes; lantern 3,565,467 →
    1,995,548).
