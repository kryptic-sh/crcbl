...
| **P7B**    | **The rasterised lighting twin, complete** (topic 18): spot and point shadows, screen-space AO, screen-space reflections, irradiance probes. Every one of the four is built and gated by a golden in `crates/crcbl/tests/render_e2e.rs`; the row stays unticked, and the reason moved twice on 2026-08-23. `apps/lantern`'s room gained a spot light (`room::spot`), which did not close it: `SHADOW_LIGHT_TILES` and `SHADOW_POINT_FACES` were the same number, so the atlas's light region was exactly one point light's cube and a room with both got one of their two maps. **That blocker is gone** — the region is seven tiles now and lantern shadows all three of its lights. What is left is not the engine but the room: the downlight's cone falls where nothing stands in it, so the third shadow is written into the atlas and occludes nothing a camera can see. Closing the row now means a composition change — an occluder in the cone — and a re-bless. Lands **before** the ray-traced path — it is what macOS, iOS and every browser run                                                                                                                        | 18                | lantern (13) renders the scene complete on `LightingPath::Rasterised`                                                                                          |
