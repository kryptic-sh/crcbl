...
| **P5C**    | **`crcbl-shell` Win32 + AppKit backends** (topic 15), moved out of P14: hand-written `extern "system"` FFI and Objective-C runtime FFI, window lifecycle, the two display modes, monitor/DPI enumeration, input, clipboard and drag-drop — each half landing with an end-to-end suite against a real desktop in CI, the treatment Wayland and X11 got at P0.5/P0.6                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 15, 12            | A window opens, flips mode and reports input on Windows and macOS, proven by a per-platform e2e job rather than by compiling                   |
...
| **P14**    | Metal + DX12 backends (MoltenVK spike gate first) — the HAL half only; the Win32/AppKit **shell** backends moved to P5C                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 9                 | Sandbox + samples on macOS/Windows                                                                                                             |
...
all until MoltenVK clears its P14 spike gate. So the F11-at-a-running-game pass
has no equivalent here, and that is a coverage gap to state rather than to
approximate — it comes back at P14, when there is something to draw with.

### What P14 keeps

Metal and DX12, the MoltenVK spike gate, and the sample/editor bring-up on both
platforms. It inherits a shell that already works there instead of writing one.
