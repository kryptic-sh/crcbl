...
| **P5B** 🟡 | **Built except the browser half.** `crcbl-jobs` ships the spawn seam, mailboxes, the ring, the work-stealing pool and `par_for`, and `apps/horde` is adopted onto it with `--workers`. The **Web Worker** spawner landed 2026-08-23 — `crcbl_jobs::workers::Workers`, queueing requests a page drains through `__crcbl_web_jobs_*`, gated by `web/tools/worker-gate.mjs` under real workers. A page implements the shim as of the same day — `web/jobs/`, driven by `web/run-jobs-e2e.sh`, which brings workers up in **a real browser** and asserts Rust ran on a stack and a thread-local of its own, with four red checks that each break their own assertion. Owed, and the reason is precise rather than effort: **no demo page announces**, so `threaded()` still answers false everywhere the site is served, and the exit criterion says a _sample's_ sim. `web/jobs/` drives `crcbl-jobs`' gate **example** on purpose, because an example cannot reach a site artifact. Closing the row needs an export in `apps/horde` that can say which thread took a chunk — its ABI reports a status code, an error string and a log queue, and its threaded and inline runs are identical frames by design, so there is nothing in a sample to assert against today. Also owed: adoption by the other three samples, and the cross-origin isolation gate on Pages — which is not a matter of effort: Pages cannot send COOP/COEP, so that criterion is met locally by `run-browser-e2e.sh` and is unmeetable on the deploy unless the `coi-serviceworker` shim is accepted. Originally: **`crcbl-jobs` + worker parity** (topic 21), moved ahead of P6–P8: the spawn seam (native `std::thread`, wasm Web Worker, single-thread fallback), latest-wins mailboxes, accumulate-then-swap input ring, work-stealing pool, `par_for` deterministic mode; **adopted by the four existing samples**; cross-origin isolation gate on Pages | 21, 10, 19        | threads-1-vs-N hash test green in CI; a sample's sim runs off the main thread in a browser; `crossOriginIsolated` asserted by the browser gate                 |
