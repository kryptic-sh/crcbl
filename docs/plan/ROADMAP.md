...
| **S1** — breakout | **WIP**  | `d747a84` scaffold (window loop, ortho camera, render graph), `71d931c` paddle + input (ECS, Server/Client, ActionMap), `5989f1b` ball + physics (PhysicsSystem, swept-sphere CCD, gravity), `495a7fb` bricks + scoring + game states                                                                                                              |
...
- **`crcbl-cli`** (`new`/`run`/`build`) and **`apps/sandbox`**, which draws a
  ...
  Wayland and X11.
- **`apps/breakout`** (S1, first 4 slices): paddle movement via `ActionMap`,
  ball/wall/paddle colliders through `PhysicsSystem` with swept-sphere CCD,
  40-brick grid with scoring and lives, game state machine
  (Waiting→Playing→Won/Lost), all running server-authoritative over
  `InMemoryTransport`. Exits: `crcbl-ui` HUD overlay and `crcbl-audio` spatial
  bounces remain for a later slice.
...
**1194 unit/integration tests** (13 new from S1), plus 26 Vulkan e2e (run on
...
| **S1**     | **Sample: breakout** — first playable; all deps (P0–P4A) exist before it starts; bounces pan spatially. **Slices 1–4 landed**: scaffold (window loop, ortho camera, render graph), paddle + input (ECS, Server/Client, ActionMap), ball + physics (PhysicsSystem, swept-sphere CCD, gravity), bricks + scoring + game states (Waiting→Playing→Won/Lost). Remaining: HUD overlay, spatial audio, e2e test.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —                 | Winnable/losable native game — 4/5 slices complete, no HUD or audio yet            |
