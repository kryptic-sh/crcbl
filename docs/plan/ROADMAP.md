...
| **P5B** 🟡 | **All three exit criteria are met; one of them is unmeetable on the deploy.** `crcbl-jobs` ships the spawn seam, mailboxes, the ring, the work-stealing pool and `par_for`; `crcbl_jobs::workers::Workers` is the Web Worker spawner, queueing requests a page drains through `__crcbl_web_jobs_*`; and `apps/horde` is adopted onto the pool. Criterion 1 — **the threads-1-vs-N comparison** — is `steering_is_bit_identical_however_many_workers_run_it` and `the_same_script_replays_bit_identically_at_every_worker_count` in `apps/horde/src/game.rs`, both plain `#[test]`, so every `build + test` leg runs them, on x86-64 and on aarch64. Criterion 2 — **a sample's sim off the main thread in a browser** — landed 2026-08-23: `web/run-horde-threads-e2e.sh` loads the horde demo on the threaded site under Xvfb and reads `__crcbl_horde_sim_threads`; measured at four threads with a pool of three, Chromium 151, Linux. Criterion 3 — **`crossOriginIsolated` asserted by the browser gate** — is `run-browser-e2e.sh`, which refuses a run whose output does not contain the check by name. **What is left is one decision.** Both browser gates run in CI as of 2026-08-23 — `ci.yml`'s `jobs-worker-e2e` installs the pinned nightly with `components: rust-src` and drives both, green on its first run in 5.0 min — on `ubuntu-latest` and no other OS. GitHub Pages cannot send COOP/COEP, so criterion 3 is unmeetable on the deploy unless the third-party `coi-serviceworker` shim is accepted; that decision is in `docs/backlog.md`, and if it is declined the criterion should be struck rather than left standing. **Adoption by all four samples is answered rather than owed:** the adoption slice measured one candidate, not four — breakout has forty bricks and asteroids at most forty-four rocks, both smaller than a single `par_for` chunk, so a pool over them would be the serial loop plus a pool. Originally: **`crcbl-jobs` + worker parity** (topic 21), moved ahead of P6–P8: the spawn seam (native `std::thread`, wasm Web Worker, single-thread fallback), latest-wins mailboxes, accumulate-then-swap input ring, work-stealing pool, `par_for` deterministic mode; **adopted by the four existing samples**; cross-origin isolation gate on Pages | 21, 10, 19        | threads-1-vs-N hash test green in CI; a sample's sim runs off the main thread in a browser; `crossOriginIsolated` asserted by the browser gate                 |
