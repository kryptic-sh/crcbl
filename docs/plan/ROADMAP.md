...
- **`crcbl sim`** (P2a): determinism harness — runs N ticks of a seed-generated
  ...
  runs, verified. A verb of `crcbl-cli`, not a crate: it was the `crcbl-sim`
  binary until 2026-08-23, when it moved behind the verb topic 11 always
  specified.
...
`crates/crcbl/tests/render_e2e.rs`, and `apps/lantern` draws sun, point and spot
shadows in one frame — the phase's own exit criterion, which its row was ticked
on later the same day. What remains is the bindless page and P7C.
