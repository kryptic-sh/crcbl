...
- **No specular IBL.** L1 irradiance answers "how much light arrives at this
  ...
  **The prefiltered half exists since 2026-08-29** as
  ...
  one committed table serves every sky. What is still missing is the image on
  the device and the term in `mesh.slang` that reads it beside the `DFG` pair.
...
| **Specular IBL: prefiltered radiance and a BRDF LUT**                             | §5 — what makes a rough metal read as metal; one rung with the sky, and it reuses energy compensation's table. **The prefiltered half is cooked (2026-08-29)**: `crcbl_shaders::sky_prefilter` is the gradient convolved against the lobe as one committed table. The upload and the shader term remain |
