...
- **No lightmaps and no baked GI — and since 2026-08-30 that is the user's rule,
  not an absence**: the sun and every scene light are dynamic and nothing bakes
  a lighting result. The candidate that satisfies it is `docs/backlog.md`'s
  runtime-traced probe volume — the same `GpuProbe` rows, filled every frame by
  a compute tracer over a GPU triangle BVH with a fixed ray pattern and no
  history.
...
| **(c) The GPU triangle BVH and its compute tracer**                                                                         | §5 — the runtime GI's engine and a gameplay raycast's; the CPU intersector is its reference. Waits on the GI decision in `docs/backlog.md`, which the no-bake rule of 2026-08-30 has narrowed to runtime-traced probes                                                                                                                                                                                                           |
...
| **One `antialiasing` row, CMAA2 in SMAA's place, then MSAA 2×/4×/8×**                                                       | [49-antialiasing.md](49-antialiasing.md)'s eighth decision — the settings row first, the depth resolve second                                                                                                                                                                                                                                                                                                                    |
