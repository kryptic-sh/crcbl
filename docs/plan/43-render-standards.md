...
**Every `forward` figure below has two full-extent clears in it, and this is
what they cost (measured 2026-09-05).** The pass opens by clearing the scene
colour and the reflectivity target over the whole extent, as `LoadOp::Clear`s
fused into its begin: putting a timestamp around them would mean giving them a
pass and a second full-target write of their own, so the millisecond figures and
the shares quoted for `forward` — here, in [44-lighting.md](44-lighting.md) and
in [47-reflections.md](47-reflections.md) — include them. What separates them
