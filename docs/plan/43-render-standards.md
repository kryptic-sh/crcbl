...
| Area                    | Here                                                                           | Owner                                                                                                                                      |
| ----------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Geometry and visibility | **ahead**                                                                      | 03 (built but for bindless and the transparent pass; rules in [rendering notes](../notes/rendering.md)), 25 (the same notes)               |
| Shadows                 | behind, ladder written                                                         | 45 (built but for static caching and ray-traced shadows; rules in [rendering notes](../notes/rendering.md), the rest in `docs/backlog.md`) |
...
- **Mesh shaders with a cluster DAG and screen-space error LOD**, and an
  ...
  `mesh_cluster.slang`, topic 25 ([rendering notes](../notes/rendering.md)). The
  comparand here is Nanite, and the honest comparison is that this is the same
  shape at a fraction of the scope: no software rasteriser for sub-pixel
  triangles, no streaming.
...
- **A physically based BRDF**, not a Blinn-Phong lobe with a roughness slider
  ...
  HDRP and Filament shade with, and the [rendering notes](../notes/rendering.md)
  carry the derivation under topic 44. **The BRDF is not this engine's PBR gap**
  — §2 and §5 are, and topic 44's ladder is what closes them.
...
2. **A second and third texture page** — **built**, and the normal one landed
   ...
   "shinier than intended", which is why it survives review — see topic 44's
   rung 2 in the [rendering notes](../notes/rendering.md), and
...
1. **The chain, built on the host and uploaded whole — built 2026-08-29.**
   ...
   and the length it lost is the roughness that topic 44's rung 4 was to put
   back ([rendering notes](../notes/rendering.md); still unbuilt, in
   `docs/backlog.md`).
...
4. **Block compression: KTX2 carrying Basis Universal, decided 2026-09-06.** The
   ...
   the two-channel normal topic 44's rung 2 asks for
   ([rendering notes](../notes/rendering.md)), `Bc4RUnorm` for a mask or a lone
   roughness — and what was open was the encoder. It is chosen, and both halves
   are new dependencies the user has taken:
   ...
   - **The transcode target is the device's answer**, resolved at load: BC7 (and
     ...
     on the day a device without BC is targeted — a capability question in the
     capability rules' sense ([backends notes](../notes/backends.md)), answered
     through `Features`, not a renderer branch.
...
- **The split-sum escape, and why the determinism rule does not block it.**
  ...
  transcendental into a table is the general escape, and topic 44's rung 3 in
  the [rendering notes](../notes/rendering.md) is where it is written down.
- **Multi-scatter energy compensation is in the frame (2026-08-27)**, and this
  ...
  specular-IBL rung above unchanged. Topic 44's rung 1, in the
  [rendering notes](../notes/rendering.md).
...
- **No lightmaps, no baked GI, and no GI at all below the ray-tracing tier — the
  ...
  that tier carries one bounce after all — the visibility-gated probe volume,
  updated every frame from the sun's reflective shadow map and weighted at
  shading by a per-probe depth map so it does not leak. On `crcbl-vk`,
...
only bounce; the rebuilt probe volume (rules in the
[rendering notes](../notes/rendering.md)) is now a leak-free bounce on every
tier, and a screen-space term on top of it would add a view-dependent,
off-screen-blind estimate of the same quantity for a pass of its own. Contact
shadows **shipped 2026-09-01** and are off this queue —
`crcbl_render::contact_shadows`, off by default; the ranking argument that put
them first is kept above as the reasoning, not as a plan. The cone trace and the
SDF path stay where they were as the RT tier's raster alternatives, not
scheduled.
...
| Tonemap curve  | ACES or AgX         | **built 2026-08-27**, ACES; the default since 2026-09-06       |
| Bloom          | Karis-average chain | **built**, off unless a view asks                              |
| Colour grading | 3D LUT              | **missing**, specified 2026-09-06; design in `docs/backlog.md` |
...
selector in its block, and the [rendering notes](../notes/rendering.md) record
why the curve is ACES rather than AgX, which needs transcendentals this
workspace's goldens cannot absorb, and why the clamp stays what a readout or a
display-referred caller asks for. The fit became `ForwardRenderer`'s default on
2026-09-06 (_The ACES default flip_ in the same notes).
...
the AA resolve. Every stage of the post chain now genuinely runs at the internal
extent, which is the whole point of the ordering the
[rendering notes](../notes/rendering.md) record. At full scale there is no
second image and no pass: the earlier stage writes the caller's target directly,
the same additive-zero shape the FXAA rung landed in.
...
bounce, because what bounces off a scene's floor is the irradiance-probe volume
and adding a second one here would count it twice.
...
1. **TAA** — `docs/backlog.md`'s _TAA is unbuilt: jitter, history, and the
   golden decision it owes_.
...
- **Deferred shading and visibility buffers** — topic 44 chose clustered forward
  (its rules are in the [rendering notes](../notes/rendering.md)), and the
  reason is transparency and MSAA, both of which §3 and topic 49's MSAA decision
  still stand on.
...
- **A second material model** — one BRDF, and every path shades with it. That
  ...
  bring the rest. Topic 44's refusal is recorded in the
  [rendering notes](../notes/rendering.md).
...
`crcbl_render::PassStats` and the per-machine baseline `docs/backlog.md`
schedules (_Profiling: five of the eight gaps are still open_) — not a sentence
saying it is cheap. The software and browser tiers pay for every pass at tens of
times the desktop cost, and they are the tiers every golden runs on, so the
desktop number alone is not a price. The same rule holds the forward pass to
topic 44's attachment budget ([rendering notes](../notes/rendering.md)).
...
millisecond figures and the shares quoted for `forward` — here and in the
...
| **(f) The shared importance and hysteresis helper — REFUSED as stated, 2026-08-31**                                                                                                                                                                                                                                                                                                                                                                                       | Topic 25 + topic 45 ([rendering notes](../notes/rendering.md)) — one scorer for cluster selection and shadow-tile priority. Built as one scorer _per plan_, not one across the two, because the two are not one function: `crcbl_render::shadow::coverage` divides by the distance to the light's **centre**, an angular radius, while `GroupCost::projected_error` divides by the distance to the group's sphere's **surface** and answers infinity inside it, a worst-case error. One helper would need a flag choosing the denominator, which is the shape this workspace refuses. The hysteresis is further apart still: the LOD band is a boolean two-budget rule per (instance, group) with its state in a device-local buffer, and the shadow band is a multi-level ladder walked on the CPU per light. What is shared is the number — `LEVEL_HOLD_RATIO` is deliberately the same fifth `ForwardRenderer::lod_hold_ratio` opens, and each names the other. What the shadow side genuinely deduplicated is its own: `coverage` is the single number both the tile ranking and the tile size read                               |
| **(g) Quality presets low / medium / high — the seam landed 2026-08-31**                                                                                                                                                                                                                                                                                                                                                                                                  | Topic 39's tier table (now in `docs/backlog.md` under _The tier table the quality presets are built from_) — every rung's rows behind three names, which is what the three-tier pricing rule reports against. `crcbl::settings::presets` is the module and `quality` is the console command: a tier is a **writer** of individual `[engine.video]` keys through `crcbl::settings::apply`, never a layer the resolution order consults, and its label is derived from what the readers answer rather than stored. It covered three keys at first — the render scale, the antialiasing tier and the fog switch; `shadow_filter` and the three SSAO keys joined on 2026-09-09, making seven and separating `medium` from `high` (`disc` against `pcss`), and the shadow-atlas, probe-volume, SSR and ray-tracing rows still have no key. Nothing selects a tier at start-up, so no golden moved. What each tier should spend on the knobs the table is silent about — the shadow cadence, the occlusion pair, anisotropy — is `docs/backlog.md`'s                                                                                        |
| **The viewer as the PBR showcase — LANDED 2026-09-06** — native drag-and-drop, a bundled shelf of Khronos CC0 models picked from a list, Suzanne on open (the viewer's milestone 4; its asset licence rule is in the [samples notes](../notes/samples.md))                                                                                                                                                                                                                | **The shelf and Suzanne-on-open shipped 2026-08-30** (`apps/viewer/src/shelf.rs`, nine models, Suzanne the default); drag-and-drop works on both hosts, X11 included since `crcbl-shell`'s `x11/xdnd.rs` receives XDND drops. The full material set followed on 2026-09-06 with rung 3's two halves: `mesh.slang` samples `mro_textures` and `emissive_textures`, and the importer fills the two columns that point at them, so all four maps a document ships now draw. What is left is `occlusionTexture.strength`, which `docs/backlog.md` carries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **LTC area lights and the fill flag — rectangles landed 2026-08-31**                                                                                                                                                                                                                                                                                                                                                                                                      | topic 44's rung 5 ([rendering notes](../notes/rendering.md)) — `crcbl_shaders::ltc` and its cooked `tables/ltc.bin`, a `Light::Rect` in the rows and the froxel grid, and the polygon integral in `mesh.slang`. Priced there on both measurable tiers: a rectangle costs 3.7× a point light on radv and 2.3× on lavapipe with a full froxel of them at 1080p. What it left — sphere, tube and disc shapes, textured lights, and the browser frame — is `docs/backlog.md`'s (`fill` reached `PointLight` and `SpotLight` on 2026-09-02)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Colour grading, DOF, lens artefacts                                                                                                                                                                                                                                                                                                                                                                                                                                       | `docs/backlog.md`'s _Colour grading, the post-tonemap LUT, is specified and unbuilt_, which specifies the first of the three — a post-tonemap 32³ `Rgba8Unorm` LUT authored as `.cube`, identity when absent, a `CameraStack` field, applied inside the tonemap pass so the resolve reads a graded luma (decided 2026-09-06). DOF and lens artefacts follow it: both want the serialized-parameter shape the LUT field settles, and neither has a reason to precede a curve and a grade                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **~~CMAA2 in SMAA's place (landed 2026-09-06)~~, then MSAA 2×/4×/8×**                                                                                                                                                                                                                                                                                                                                                                                                     | The antialiasing ladder's eighth decision — the settings row landed 2026-08-30 and CMAA2 with SMAA's retirement 2026-09-06; what remains is the MSAA rungs and the depth resolve they need (`docs/backlog.md`, _MSAA was reopened rather than reversed_)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **The probe volume moves — scrolling landed 2026-09-05** — the visibility maps and the clipmap landed 2026-09-02, the sun's RSM updater 2026-09-04 with lantern's and shard's bakes gone with it, the punctual producer the same day, and the clipmap's whole-step scroll with recapture the day after                                                                                                                                                                    | the irradiance-probe decision of 2026-08-30 (rules in the [rendering notes](../notes/rendering.md)) — leak-free, one bounce, dynamic sun and lamps on every tier. Both producers are built, so lantern's coloured wall tints the plaster beside it again, and `ForwardRenderer::follow_probe_volume` re-centres every level on a tracked point by whole steps with `probe_capture::recapture` taking the exposed rows again, so a probe the camera leaves behind is captured where it lands. What the scroll still owes — the step's device drain, the recapture's transients and a caller that scrolls in anger — is `docs/backlog.md`'s. The traced updater rides on (c)                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Temporal SSR, TAA, temporal upscaling                                                                                                                                                                                                                                                                                                                                                                                                                                     | §9's pass is built; each is its own rung now. **SSGI is not among them** — it was withdrawn from this ordering on 2026-08-30, because the irradiance-probe volume (rules in the [rendering notes](../notes/rendering.md)) is a leak-free bounce on every tier where SSGI approximates the same thing in screen space on one; §5 records the withdrawal                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

### Foundation (c): the acceleration-structure seam, in outline

Four things defer to row (c) above — `ROADMAP.md`'s P7C, sundial's milestone 5
and alcove's ray-traced rung (both in `docs/backlog.md` under _Ray tracing and
the acceleration structures are unbuilt_), and lantern's both-paths comparison
at S4B — and until 2026-09-06 the only thing any of them could point at was a
capability name. This is what they point at instead. It is an outline of the
seam's shape, not a design of the passes above it: the passes are P7C's, and P7C
is scheduled after [53-transparency.md](53-transparency.md), the raster ladder's
last rung.
...
- **`Capability::RayQuery`.** One capability, answered by a `match` like every
  other in `crcbl_hal::Capability`. `crcbl-webgpu` answers it with an
  `ApiAbsence` divergence — WebGPU has no ray tracing at any size — which is the
  same shape as its `BindlessDescriptorArray` answer and is permanent rather
  than a gap.
