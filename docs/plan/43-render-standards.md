...
- **No specular IBL.** L1 irradiance answers "how much light arrives at this
  ...
  **The prefiltered half exists since 2026-08-29** as
  ...
  cone it gathers rather than the one mirror direction. What is still missing is
  the `DFG` pair as an image and the `f0 · scale + bias` that scales it where
  Schlick does today.
...
| **Specular IBL: prefiltered radiance and a BRDF LUT**                             | §5 — what makes a rough metal read as metal; one rung with the sky, and it reuses energy compensation's table. **The prefiltered half is in the frame (2026-08-29)**: `crcbl_shaders::sky_prefilter` is the gradient convolved against the lobe as one committed table, and `ssr.slang`'s miss fallback reads the sky through it at the surface's roughness. The `DFG` pair as an image and the term that scales the sky by it remain |
