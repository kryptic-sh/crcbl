...
| `brightness`   | scalar multiplier applied in the tonemap pass                   | **Nothing.** `crcbl_shaders::tonemap::TonemapParams` carries `exposure`, `curve` and `auto_exposure`, and brightness is none of them; it would be a fourth field. The `auto_exposure` lane is what makes that more than a rename — with it set the pass takes its exposure from the reduce's buffer, so a player's brightness has to compose with a value the frame chose for itself. |
