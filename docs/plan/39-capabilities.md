...
| ----------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `quality`               | `low` \| `medium` \| `high` \| `ultra` \| `custom` | **Nothing.** No preset; `apps/options` is the settings screen since 2026-08-28 and offers none.                                                                                                                                                                                                                        |
| `anti_aliasing`         | `off` \| `fxaa` \| `smaa`                          | **Built as a boolean (2026-08-27)**, and the key is spelled `antialiasing`: `RenderEffects::ANTIALIASING` is the `off`/`fxaa` pair, FXAA is in `DEFAULT_STACK`, and the `smaa` rung has nothing — [49-antialiasing.md](49-antialiasing.md).                                                                            |
...
| `anisotropic_filtering` | `1` \| `2` \| `4` \| `8` \| `16`                   | **Nothing**, and it is the first key of [43-render-standards.md](43-render-standards.md)'s filtering rung: `SamplerDesc::anisotropy` and `Limits::max_sampler_anisotropy` already exist, so it is a value key on `render_scale`'s pattern, clamped to the device's limit.                                              |
