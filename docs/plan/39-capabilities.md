...
| `anisotropic_filtering` | `1` \| `2` \| `4` \| `8` \| `16`                   | **Built 2026-08-29** as a value key on `render_scale`'s pattern: `crcbl::settings::anisotropic_filtering` reads it, `1` through the desktop preset's sixteen, and `ForwardRenderer::set_anisotropy` clamps it to the device's own `Limits::max_sampler_anisotropy`. The `apps/options` row is what is left.            |
