...
| `anti_aliasing`         | `off` \| `fxaa` \| `smaa`                          | **Built as two booleans (2026-08-30)**, and the keys are spelled `antialiasing` and `smaa`: `RenderEffects::ANTIALIASING` is the `off`/`fxaa` pair and FXAA is in `DEFAULT_STACK`; `RenderEffects::SMAA` is the `smaa` rung, drawn and off by default. They share one resolve slot rather than stacking, which is the three-value domain arriving as two bits — [49-antialiasing.md](49-antialiasing.md). |
