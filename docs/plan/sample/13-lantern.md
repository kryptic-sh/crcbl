...
- **Rule 12's other half: a second path actually drawn** (2026-08-14). Saying
  ...
  own selectors, once with the features that select anything above
  `ForcedPaths`'s floor withheld — and holds **both arms to the one golden**: a
  lesser path is a constraint on data layout rather than a separate renderer, so
  a difference between the arms is a bug in the better path and a per-path
  reference is what would bless it. The subtraction is done by
  `crcbl::engine::ForcedPaths::optional_features`, the same function `--force-*`
  goes through, so it cannot drift from the flags a selector reads. Measured:
  radv resolves the arms to `MeshShader / Bindless` and
  `IndirectPerBatch / ArrayPages`, and llvmpipe reports mesh shading and
