...
- Enemies: 2–3 types, dumb seek movement + separation (cheap flocking-lite, no
  pathfinding), contact damage, HP, on-death despawn.
...
1. 1k enemies seeking player, culling stats on HUD (stage 5-ish start, scene
   from file).
2. Combat loop complete (damage, XP, level-up UI) — after stage 6.
...
- 10k enemies at 60 fps render / 60 Hz tick on the reference Linux machine
  (numbers recorded here, revisited per backend in stages 8–9 — Tier B/wasm gets
  its own smaller recorded budget).
