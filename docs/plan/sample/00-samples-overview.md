...
| --- | ---------------------------- | -------------------- | ---------------------------------------------------------------------------- |
| 01  | [breakout](01-breakout.md)   | 4                    | 2D path, minimal ECS, in-memory server loop, game UI                         |
| 02  | [asteroids](02-asteroids.md) | 4 (nicer after 5)    | Entity churn, generational ids, spawn/destroy sweeps                         |
| 03  | [viewer](03-viewer.md)       | 5 (UI after 6)       | Asset pipeline as a _usable tool_, camera, inspector                         |
| 04  | [horde](04-horde.md)         | 5                    | GPU-driven renderer at scale, flat CPU cost claim                            |
| 05  | [towers](05-towers.md)       | 7 (wasm after 9)     | **Flagship**: everything — editor content, co-op multiplayer, browser client |
| 06  | [arena](06-arena.md)         | post-MVP             | Client prediction driver — pulls netcode forward                             |

Order is dependency order, not build-one-then-next: 01–04 stay tiny (days, not
weeks, each). 05 is the real game and the long-lived dogfood. 06 exists to force
post-MVP netcode work and is explicitly not started before MVP ends.
...
3. **Scenes from files** (once stage 5 exists): samples load `.scn.ron`, and
   after stage 7 their scenes are maintained _in the editor_. Hand-edited scene
