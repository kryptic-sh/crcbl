...
**The map is a `.scn/` directory.** `apps/puppet/assets/scenes/blockout.scn/` is
the blockout — a header, an `env.ron` whose ambient is the light the frame
actually uses, and three chunk files: the surfaces, the spawn and the sun.
`apps/puppet/src/map.rs` is its loader, and `--scene <DIR>` walks a different
one; a directory that is not a scene is refused by key, line and column, and a
scene without puppet's chunks by the chunk it is missing. The chunk row —
`map::Surface`, a `crcbl-greybox` primitive with the collider that is the same
surface — is **the sample's own type** rather than an engine one, on
`crcbl_scene::scn::chunk_of`'s terms: a component is whatever its game says it
is, and one consumer is not an engine type. `apps/breakout`'s `Brick` sits on
the same side of that line. What is still owed against the Scope is
"editor-authored": `apps/editor` now exists and opens puppet's map through
`crcbl_puppet::map::register_components`, but the committed directory was not
authored in it — it is generated by `Scene::save` from the constants in `map.rs`
and held to them byte for byte by `the_committed_map_is_what_the_writer_writes`.

**One thing differs from the Scope above and it is a decision rather than a
gap.** The character is a **greybox humanoid authored in code** —
