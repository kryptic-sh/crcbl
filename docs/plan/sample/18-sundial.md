...
2. **Normal-offset bias and the acne/peter-panning pair**, checkable at the
   ...
   **2026-09-04** gave the acne half a reading of its own, where before only
   ...
   `;` `'`, with `9` `0` and `7` `8` walking the same two counts in coarse steps
   out to the far end of either range, to the pause panel's `BIAS` and
   `NORMAL OFFSET` rows and to the debug overlay's `shadow filter` section. They
   are **on the page** as well — two sliders on `/demos/sundial/` over
   `__crcbl_sundial_bias` and `__crcbl_sundial_normal_offset`, each track
   spanning the ceiling the engine declares, and both counts on the `[HUD]`
   line, so the browser gate reads a drag off the console's cell rather than off
   the canvas.
   ...
   **2026-09-05 widened it from one setup to four rows over three rungs**, each
   ...
   pavement past it away together — `176.00`/`173.33` at 50 texels,
   ...
   run reports all of them together because the arms are read into one list of
   faults rather than one assertion each.
